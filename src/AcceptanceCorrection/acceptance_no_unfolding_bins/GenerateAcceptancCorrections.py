...
# ==============================================================================
def e906_chuck_cuts(tree: uproot.TTree, cut=3.0, beam_offset: float = 1.6):
    """
    ...
    # Widened mass and xF boundaries to prevent floating-point edge clipping
    dimuon_cut_2111_v42 = (
...
def get_optimal_legend_position(hists, y_min=0.75, y_max=0.88):
    ...
    Finds whether the highest data points exist on the left or right 
    half of the plot and returns coordinates for the opposite side.
    """
    ...
    for h in hists:
        ...
        def get_side_max(start_bin, end_bin):
            ...
            for i in range(start_bin, end_bin + 1):
                ...
                # Ignore empty bins and unphysical/noisy bins (SNR < 0.66)
    ...
    # Default to top-right if flat or empty
    if left_max == -np.inf and right_max == -np.inf:
        return (0.65, y_min, 0.88, y_max)

    # Place legend on the side with the LOWER maximum value
    if left_max > right_max:
        # Taller data on the left -> Put legend on the Top Right
        return (0.65, y_min, 0.88, y_max)
    else:
        # Taller data on the right -> Put legend on the Top Left
        return (0.15, y_min, 0.38, y_max)
...
def process_acceptance_sliced(x_var, x_edges, slice_var, slice_edges, t_lh2_th, t_lh2_ac, t_ld2_th, t_ld2_ac, out_file):
    ...
    for i in range(n_bins):
        ...
        lh2_th_h, lh2_th_err = get_weighted_histogram(t_lh2_th[x_var][m_lh2_th], t_lh2_th.ReWeight[m_lh2_th], x_edges)
        lh2_ac_h, lh2_ac_err = get_weighted_histogram(t_lh2_ac[x_var][m_lh2_ac], t_lh2_ac.ReWeight[m_lh2_ac], x_edges)
        ...
        ld2_th_h, ld2_th_err = get_weighted_histogram(t_ld2_th[x_var][m_ld2_th], t_ld2_th.ReWeight[m_ld2_th], x_edges)
        ld2_ac_h, ld2_ac_err = get_weighted_histogram(t_ld2_ac[x_var][m_ld2_ac], t_ld2_ac.ReWeight[m_ld2_ac], x_edges)
        ...
        leg_coords = get_optimal_legend_position([h_ratio_LH2, h_ratio_LD2, h_ratio_combine], y_min=0.70, y_max=0.88)
...
def process_integrated_1D(var_name, var_edges, t_lh2_th, t_lh2_ac, t_ld2_th, t_ld2_ac, out_file):
    ...
    h_ratio_LD2 = make_th1f(f"h_ratio_LD2_{binName}", title, var_edges_root, ld2_ratio, lh2_ratio_err, out_dir)
...
def plot_kinematics_separated(var_name, title_suffix, file_prefix, var_edges, t_lh2_th, t_lh2_ac, t_ld2_th, t_ld2_ac, out_dir):
    ...
    # Intelligent Legend Placement
    leg_coords_th = get_optimal_legend_position([h_lh2_th, h_ld2_th], y_min=0.75, y_max=0.88)
    leg_th = ROOT.TLegend(*leg_coords_th)
    leg_th.SetBorderSize(0); leg_th.SetFillColor(ROOT.kWhite)
    leg_th.AddEntry(h_lh2_th, "LH2 4#pi", "l")
    leg_th.AddEntry(h_ld2_th, "LD2 4#pi", "l")
    leg_th.Draw()

    c_th.Write(); c_th.SaveAs(f"yield_th_{var_name}_{file_prefix}.pdf")
    h_lh2_th.Write(); h_ld2_th.Write()
    ...
    # Intelligent Legend Placement
    leg_coords_ac = get_optimal_legend_position([h_lh2_ac, h_ld2_ac], y_min=0.75, y_max=0.88)
    leg_ac = ROOT.TLegend(*leg_coords_ac)
    leg_ac.SetBorderSize(0); leg_ac.SetFillColor(ROOT.kWhite)
    leg_ac.AddEntry(h_lh2_ac, "LH2 Clean", "l")
    leg_ac.AddEntry(h_ld2_ac, "LD2 Clean", "l")
    leg_ac.Draw()

    c_ac.Write(); c_ac.SaveAs(f"yield_ac_{var_name}_{file_prefix}.pdf")
    h_lh2_ac.Write(); h_ld2_ac.Write()
...
def process_dp_binned_xf(dpEdge, dp2Edge, xF_edges, t_lh2_th, t_lh2_ac, t_ld2_th, t_ld2_ac, out_file):
    ...
    for i in range(n_bins):
        ...
        binName = f"xF_bin{i}"
        title_suffix = f"{val_low:.2f} #leq x_{{F}} < {val_high:.2f}"
...
def create_split_ratio_canvas(var_name, var_edges, t_lh2, t_ld2, mask_lh2, mask_ld2, plot_name, title, x_title, out_dir):
    ...
    # Intelligent Legend Placement
    leg_coords = get_optimal_legend_position([h_lh2, h_ld2], y_min=0.75, y_max=0.88)
    leg = ROOT.TLegend(*leg_coords)
    leg.SetBorderSize(0)
...
def main():
    ...
    print("Calculating derived kinematics (pT, pT2, dpx2, dpy2)...")
