...
// Additional headers needed for the fit error band
...
void acceptance_target_2(){
    gStyle->SetOptStat(0);
    gStyle->SetOptFit(1);
    TString MCdir = "~/github/e906-development/ROOTFiles/Hugo/";
    ...
    for (int i=0; i <nxF_CS; i++){
        ...
        //LH2_thrownTree->Draw("sqrt(dpx*dpx+dpy*dpy)>>h_LH2_thrown_"+binName,"ReWeight"*(MCcut&&cut4pi_H), "goff");
        //LH2_acceptTree->Draw("sqrt(dpx*dpx+dpy*dpy)>>h_LH2_accept_"+binName,"ReWeight"*(allCut), "goff");
        ...
        //h_ratio_LH2->GetYaxis()->SetRangeUser(0, 0.1);
        h_ratio_LH2->Draw();
        ...
        //LD2_thrownTree->Draw("sqrt(dpx*dpx+dpy*dpy)>>h_LD2_thrown_"+binName,"ReWeight"*(MCcut&&cut4pi_D), "goff");
        //LD2_acceptTree->Draw("sqrt(dpx*dpx+dpy*dpy)>>h_LD2_accept_"+binName,"ReWeight"*(allCut), "goff");
        ...
        //c1->SaveAs(TString::Format("acceptance_xT_bin%i.png",i));
        ...
        // h_ratio_acceptance->GetYaxis()->SetRangeUser(0.8, 1.2);
        
        //--- MODIFICATIONS START HERE ---

        // 3. Create a TGraphErrors object to hold the confidence interval band.
        //    Using the histogram as an argument is a convenient way to set the x-points.
        //TGraphErrors *g_error_band = new TGraphErrors(h_ratio_acceptance);

        // 4. Get the 68% confidence intervals (1-sigma) for the fit function.
        //    This populates the TGraphErrors object with the fit errors.
        //(TVirtualFitter::GetFitter())->GetConfidenceIntervals(g_error_band);

        // 5. Set the visual style for the error band.
        //g_error_band->SetFillColor(kPink - 9); // Use a light, transparent pink
        //g_error_band->SetFillStyle(3004);      // A semi-transparent fill style (dots)
        //g_error_band->SetLineColor(kPink - 9); // Set line color to match fill

        // 6. Draw the error band. The "E3" option draws it as a filled area.
        //g_error_band->Draw("E3 SAME");
        

        // 1. Draw the histogram with its errors first to set the axis ranges.
        h_ratio_acceptance->Draw("E1");
        
        // 2. Perform the fit. The "S" option is crucial to get the TFitResultPtr.
        //    "Q" suppresses the fit results from being printed to the console.
        TFitResultPtr r = h_ratio_acceptance->Fit("pol0", "SQ");
        ...
        // 5. Set the visual style for the error band.
        g_error_band->SetFillColor(kPink - 9); // Use a light pink color
        g_error_band->SetFillStyle(3004);      // A semi-transparent fill style (dots)

        // 6. Draw the error band. The "E3" option draws it as a filled area.
        g_error_band->Draw("E3 SAME");
        
        // 7. Redraw the data points on top of the error band for clarity.
        h_ratio_acceptance->Draw("E1 SAME");

        // The fit line is drawn by default by the Fit() command. If you need to ensure
        // it's on top, you can uncomment the following line:
        // h_ratio_acceptance->GetFunction("pol0")->Draw("SAME");
        
        h_ratio_acceptance->Write();
        c2->SaveAs(TString::Format("acceptance_ratio_xT_bin%i.pdf",i));
