...
# =============================================================================
SYS_FRAC_ROADSET_LH2 = 0.0  # Replace with CSV outputs later
SYS_FRAC_ROADSET_LD2 = 0.0  # Replace with CSV outputs later
...
class DYCrossSectionAnalyzer:
    ...
    def calculate_and_plot_cross_section(self, h_sub_dict, target_label, global_constant, use_true_pt=False, observable="pT"):
        """Builds single differential cross-sections vs pT or pT^2, incorporating PsiP & Roadset systematic contaminations."""
...
""" % (target_label, target_label)
        ...
        for i_pt in range(n_pt_bins):
            ...
            sum_xsec = dsigma_val
            sys_roadset = sys_frac_roadset * sum_xsec
            ...
            if observable == "pT2" and sum_xsec_bw > 0:
                sys_roadset_bw = sys_frac_roadset * sum_xsec_bw
                sys_acc_total_bw = (acceptance_err / acceptance) * sum_xsec_bw
                total_sys_err_bw = math.sqrt(sys_tot_yield_bw**2 + sys_psip_cont_bw**2 + sys_acc_total_bw**2 + sys_roadset_bw**2)
    ...
    def generate_overlay_plot(self, use_true_pt=False, observable="pT"):
        ...
        note_text = "#splitline{10% global uncertainty due to the integrated luminosity is not included in the error bands,}{but bin-by-bin roadset systematic uncertainties are included.}"
        lumi_note.DrawLatex(0.18, 0.16, note_text)
    ...
    def generate_ratio_plot(self, use_true_pt=False, observable="pT"):
        ...
        for i_pt in range(len(bins) - 1):
            ...
            # Center systematic band at 0.0
            g_ratio_sys.SetPoint(pt_idx, bin_center, 0.0)
        ...
        if has_bw:
            ...
            for i in range(g_xsec_bw_lh2.GetN()):
                ...
                # Center systematic band at 0.0
                g_ratio_sys_bw.SetPoint(pt_idx_bw, sys_x, 0.0)
        ...
        if g_ratio_stat.GetN() > 0:
            ...
            g_ratio_stat.Fit(fit_func, "Q0")  # Fit stats for values
            ...
            if has_bw:
                ...
                g_ratio_stat_bw.Fit(fit_func_bw, "Q0")
            ...
            line1 = ROOT.TLine(0.0, 1.0, x_max, 1.0)
            line1.SetLineStyle(2); line1.SetLineColor(ROOT.kGray+2); line1.SetLineWidth(2); line1.Draw("SAME")
            line0 = ROOT.TLine(0.0, 0.0, x_max, 0.0)
            line0.SetLineStyle(2); line0.SetLineColor(ROOT.kGray+1); line0.SetLineWidth(1); line0.Draw("SAME")
            ...
            latex_fit.DrawLatex(0.1 * x_max, 1.25, f"Jacobian Best Fit: {fit_val:.4f} #pm {fit_err:.4f}")

            if has_bw:
                latex_fit.SetTextColor(ROOT.kBlue)
                latex_fit.DrawLatex(0.1 * x_max, 1.15, f"Bin Width Best Fit: {fit_val_bw:.4f} #pm {fit_err_bw:.4f}")
            ...
            lumi_note = ROOT.TLatex(); lumi_note.SetNDC(True); lumi_note.SetTextFont(43); lumi_note.SetTextSize(15)
            lumi_note.SetTextColor(ROOT.kBlack); lumi_note.SetTextAlign(11)
            note_text = "#splitline{10% global uncertainty due to the integrated luminosity is not included in the error bands,}{but bin-by-bin roadset systematic uncertainties are included.}"
            lumi_note.DrawLatex(0.18, 0.16, note_text)
    ...
    def generate_combined_ratio_overlay_plot(self, use_true_pt=False, observable="pT"):
        ...
        mg_bottom.SetMinimum(-0.5)
        mg_bottom.SetMaximum(1.8)
        ...
        if has_bw:
            fit_func_bw.SetLineColor(ROOT.kBlue); fit_func_bw.SetLineWidth(2); fit_func_bw.Draw("SAME")
            latex_fit.SetTextColor(ROOT.kBlue)
            latex_fit.DrawLatex(0.1 * x_max, 1.25, f"Bin Width Best Fit: {fit_val_bw:.4f} #pm {fit_err_bw:.4f}")
        else:
            fit_func.SetLineColor(ROOT.kRed); fit_func.SetLineWidth(2); fit_func.Draw("SAME")
            latex_fit.SetTextColor(ROOT.kRed)
            latex_fit.DrawLatex(0.1 * x_max, 1.25, f"Jacobian Best Fit: {fit_val:.4f} #pm {fit_err:.4f}")
