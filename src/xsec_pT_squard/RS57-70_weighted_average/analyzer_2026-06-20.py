...
""" % (target_label, target_label)
    ...
    def generate_combined_ratio_overlay_plot(self, use_true_pt=False, observable="pT"):
        """Creates a split canvas with cross-section overlays on top and the ratio plot on bottom. Includes direct bin width implementations."""
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        y_max_val = 0
        graphs_to_check = [g_xsec_lh2, g_xsec_ld2] + ([g_xsec_bw_lh2, g_xsec_bw_ld2] if has_bw else [])
        err_graphs = [g_sys_lh2, g_sys_ld2] + ([g_sys_bw_lh2, g_sys_bw_ld2] if has_bw else [])
        ...
        leg_top.AddEntry(g_xsec_lh2_clone, "pp (Jacobian)", "pl"); leg_top.AddEntry(g_sys_lh2_clone, "pp Sys (Jacobian)", "f")
        leg_top.AddEntry(g_xsec_ld2_clone, "pd (Jacobian)", "pl"); leg_top.AddEntry(g_sys_ld2_clone, "pd Sys (Jacobian)", "f")
        if has_bw:
            leg_top.AddEntry(g_xsec_bw_lh2_clone, "pp (Bin Width)", "pl"); leg_top.AddEntry(g_sys_bw_lh2_clone, "pp Sys (Bin Width)", "f")
            leg_top.AddEntry(g_xsec_bw_ld2_clone, "pd (Bin Width)", "pl"); leg_top.AddEntry(g_sys_bw_ld2_clone, "pd Sys (Bin Width)", "f")
        leg_top.Draw()
