...
class DYCrossSectionAnalyzer:
    ...
    def calculate_and_plot_cross_section(self, h_sub_dict, target_label, global_constant, use_true_pt=False, observable="pT"):
        """Builds single differential cross-sections vs pT or pT^2, incorporating PsiP systematic contamination, and compares calculations for pT2."""
        ...
        acc_dir_name = f"Integrated_{observable}"
        acc_dir = acc_file.Get(acc_dir_name)
        if not acc_dir:
            print(f"CRITICAL ERROR: TDirectory '{acc_dir_name}' not found in acceptance file.")
            sys.exit(1)
            
        h_acc_name = f"h_ratio_{target_label}_{acc_dir_name}"
        h_acc_1d = acc_dir.Get(h_acc_name)
        
        if not h_acc_1d and target_label == "LD2":
            h_acc_1d = acc_dir.Get(f"h_ratio_LH2_{acc_dir_name}")
            
        if not h_acc_1d:
            print(f"CRITICAL ERROR: Acceptance histogram '{h_acc_name}' not found.")
            sys.exit(1)
        ...
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"

        if observable == "pT2":
            bins = config.PT2_BINS
            x_title = "p_{T}^{2} [GeV^{2}]"
            y_title = "d#sigma/dp_{T}^{2} [nb/GeV^{2}/Nucleus]"
        else:
            bins = config.PT_BINS
            x_title = "p_{T} [GeV]"
            y_title = "d#sigma/dp_{T} [nb/GeV/Nucleus]"
...
""" % (target_label, target_label)
        ...
        # New Graphs for Direct Bin Width approach
        g_xsec_bw = ROOT.TGraphErrors(); g_xsec_bw.SetName(f"g_xsec_bw_{target_label}{suffix}")
        g_sys_bw = ROOT.TGraphErrors(); g_sys_bw.SetName(f"g_sys_bw_{target_label}{suffix}")

        h1_xsec = ROOT.TH1D(f"h1_xsec_{target_label}{suffix}", f"Single Differential Cross Section {target_label};{x_title};{y_title}", n_pt_bins, bins)
        h1_sys = ROOT.TH1D(f"h1_sys_{target_label}{suffix}", f"Systematic Error {target_label};{x_title};{y_title}", n_pt_bins, bins)
        ...
        for i_pt in range(n_pt_bins):
            ...
            if observable == "pT2":
                pt2_min, pt2_max = config.PT2_BINS[i_pt], config.PT2_BINS[i_pt+1]
                pt2_width = pt2_max - pt2_min
                
                # Approach 1: Apply transformation Jacobian -> dsigma/dpT^2 = dsigma/dpT / (2 * <pT>)
                dsigma_val = bin_dsigma_dpt / (2.0 * actual_pt_center)
                stat_unc = (Y_sub_stat_err / Y_sub) * dsigma_val
                sys_tot_yield = (Y_sub_sys_err / Y_sub) * dsigma_val
                sys_psip_cont = psip_ratio * dsigma_val
                
                # Approach 2: Direct Bin Width Approach
                bin_dsigma_dpt2 = numerator / (pt2_width * acceptance)
                sum_xsec_bw = bin_dsigma_dpt2
                stat_unc_bw = (Y_sub_stat_err / Y_sub) * sum_xsec_bw
                sys_tot_yield_bw = (Y_sub_sys_err / Y_sub) * sum_xsec_bw
                sys_psip_cont_bw = psip_ratio * sum_xsec_bw

                plot_x_geom = (pt2_min + pt2_max) / 2.0
                plot_x = actual_pt_center**2 if use_true_pt else plot_x_geom
                sys_x = plot_x_geom
                sys_x_err = pt2_width / 2.0
            else:
                dsigma_val = bin_dsigma_dpt
                stat_unc = (Y_sub_stat_err / Y_sub) * dsigma_val
                sys_tot_yield = (Y_sub_sys_err / Y_sub) * dsigma_val
                sys_psip_cont = psip_ratio * dsigma_val
                ...
                sys_x = pt_center
                sys_x_err = pt_width / 2.0

            sum_xsec = dsigma_val
            
            if psip_ratio > 0.0 and not use_true_pt and observable == "pT":
            ...
            # Populate standard/Jacobian approach
            if sum_xsec > 0:
                ...
                if (max_err_for_range / sum_xsec) <= 0.99: 
                    g_sys.SetPoint(point_idx, sys_x, sum_xsec)
                    g_sys.SetPointError(point_idx, sys_x_err, total_sys_err)
            ...
            # Populate new Bin Width approach
            if observable == "pT2" and sum_xsec_bw > 0:
                sys_acc_total_bw = (acceptance_err / acceptance) * sum_xsec_bw
                total_sys_err_bw = math.sqrt(sys_tot_yield_bw**2 + sys_psip_cont_bw**2 + sys_acc_total_bw**2)
                max_err_for_range_bw = max(stat_unc_bw, total_sys_err_bw)
                
                if (max_err_for_range_bw / sum_xsec_bw) <= 0.99: 
                    g_sys_bw.SetPoint(point_idx_bw, sys_x, sum_xsec_bw)
                    g_sys_bw.SetPointError(point_idx_bw, sys_x_err, total_sys_err_bw)
                    
                    g_xsec_bw.SetPoint(point_idx_bw, plot_x, sum_xsec_bw)
                    g_xsec_bw.SetPointError(point_idx_bw, 0.0, stat_unc_bw)
                    
                    y_high_bw = sum_xsec_bw + max_err_for_range_bw
                    y_low_bw  = sum_xsec_bw - max_err_for_range_bw
                    
                    if y_low_bw <= 0: y_low_bw = sum_xsec_bw * 0.5 
                    if y_high_bw > y_max_data: y_max_data = y_high_bw
                    if y_low_bw < y_min_data: y_min_data = y_low_bw
                    
                    point_idx_bw += 1
                
        if g_xsec.GetN() > 0:
            c_xsec = ROOT.TCanvas(f"c_xsec_{target_label}{suffix}", "", 800, 600)
            c_xsec.SetLeftMargin(0.16); c_xsec.SetBottomMargin(0.14)
            c_xsec.SetTickx(1); c_xsec.SetTicky(1)
            
            mg = ROOT.TMultiGraph()
            mg.SetTitle(f";{x_title};{y_title}")
            ...
            # Widen legend to fit potential additional entries
            leg = ROOT.TLegend(0.45 if observable == "pT2" else 0.65, 0.75, 0.88, 0.88)
            leg.SetBorderSize(0)
            
            # Baseline (Jacobian) Plot Styling
            g_sys.SetMarkerSize(0); g_sys.SetLineColor(ROOT.kRed); g_sys.SetFillColorAlpha(ROOT.kPink - 9, 0.5); g_sys.SetFillStyle(1001)
            sys_legend_label = "Sys. Unc. (Jacobian)" if observable == "pT2" else "Systematic Unc."
            mg.Add(g_sys, "2"); leg.AddEntry(g_sys, sys_legend_label, "f")

            g_xsec.SetMarkerStyle(20); g_xsec.SetMarkerColor(ROOT.kRed); g_xsec.SetLineColor(ROOT.kRed)
            data_legend_label = f"Data ({target_label}) - Jacobian" if observable == "pT2" else f"Data ({target_label})"
            mg.Add(g_xsec, "P"); leg.AddEntry(g_xsec, data_legend_label, "lep")
            
            # Direct Bin Width Plot Styling
            if observable == "pT2" and g_xsec_bw.GetN() > 0:
                g_sys_bw.SetMarkerSize(0); g_sys_bw.SetLineColor(ROOT.kBlue); g_sys_bw.SetFillColorAlpha(ROOT.kAzure + 1, 0.5); g_sys_bw.SetFillStyle(1001)
                mg.Add(g_sys_bw, "2"); leg.AddEntry(g_sys_bw, "Sys. Unc. (Bin Width)", "f")

                g_xsec_bw.SetMarkerStyle(21); g_xsec_bw.SetMarkerColor(ROOT.kBlue); g_xsec_bw.SetLineColor(ROOT.kBlue)
                mg.Add(g_xsec_bw, "P"); leg.AddEntry(g_xsec_bw, f"Data ({target_label}) - Bin Width", "lep")
            
            mg.Draw("A"); mg.GetXaxis().CenterTitle(); mg.GetYaxis().CenterTitle() 
            
            x_limit = 3.5 if observable == "pT2" else 2.0
            mg.GetXaxis().SetLimits(0.0, x_limit); mg.GetXaxis().SetTitleOffset(1.3); mg.GetYaxis().SetTitleOffset(1.7)
            leg.Draw()
            ...
            c_xsec.SaveAs(f"CrossSection_{target_label}{suffix}.pdf")
            
            dir_xsec.cd()
            g_xsec.Write(); g_sys.Write(); h1_xsec.Write(); h1_sys.Write(); c_xsec.Write()
            if observable == "pT2" and g_xsec_bw.GetN() > 0:
                g_xsec_bw.Write(); g_sys_bw.Write()
            c_xsec.Close()

        if not use_true_pt and observable == "pT":
    ...
    def generate_overlay_plot(self, use_true_pt=False, observable="pT"):
        """Combines and saves LH2 and LD2 single differential cross-sections onto a single plot canvas. Maps both calculation approaches for pT2."""
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        canvas = ROOT.TCanvas(f"canvas_overlay_Targets_{suffix}", "Cross-Section Comparison", 800, 600)
        canvas.SetLeftMargin(0.16); canvas.SetBottomMargin(0.14)
        canvas.SetTickx(1); canvas.SetTicky(1)

        legend = ROOT.TLegend(0.45 if observable == "pT2" else 0.65, 0.65, 0.88, 0.88)
        ...
        if observable == "pT2":
            x_max = 3.5
            x_title = "p_{T}^{2} [GeV^{2}]"
            y_title = "d#sigma/dp_{T}^{2} [nb/GeV^{2}]"
            title_obs = "p_{T}^{2}"
        else:
            x_max = 2.0
            x_title = "p_{T} [GeV]"
            y_title = "d#sigma/dp_{T} [nb/GeV]"
            title_obs = "p_{T}"

        h_frame = canvas.DrawFrame(0.0, 0.0, x_max, 1.0)
        h_frame.SetTitle(f"DY Absolute Cross-Section Vs {title_obs}")
        h_frame.GetXaxis().SetTitle(x_title); h_frame.GetXaxis().CenterTitle()
        h_frame.GetXaxis().SetTitleOffset(1.3); h_frame.GetYaxis().SetTitle(y_title)
        h_frame.GetYaxis().CenterTitle(); h_frame.GetYaxis().SetTitleOffset(1.7)  

        # Target formatting: (Label, Jacobian Color, BW Color)
        targets = [("LH2", ROOT.kBlue, ROOT.kAzure+1), ("LD2", ROOT.kRed, ROOT.kMagenta)]
        y_min, y_max = 1e9, -1e9

        for target, color_jacob, color_bw in targets:
            ...
            # Plot Jacobian approach
            g_sys_clone = g_sys.Clone(f"g_sys_clone_{target}{suffix}")
            g_sys_clone.SetLineColor(color_jacob); g_sys_clone.SetFillColorAlpha(color_jacob, 0.35)
            g_sys_clone.SetFillStyle(1001); g_sys_clone.SetMarkerSize(0)

            g_xsec_clone = g_xsec.Clone(f"g_xsec_clone_{target}{suffix}")
            g_xsec_clone.SetLineColor(color_jacob); g_xsec_clone.SetMarkerColor(color_jacob)
            ...
            legend.AddEntry(g_xsec_clone, f"{target} Data (Jacobian)", "pl")
            
            for i in range(g_xsec.GetN()):
                y = g_xsec.GetY()[i]; ey = g_sys.GetErrorY(i)
                if y > 0:
                    if (y - ey) < y_min and (y - ey) > 0: y_min = y - ey
                    if (y + ey) > y_max: y_max = y + ey
            
            # Plot BW approach if present
            if observable == "pT2":
                g_xsec_bw = dir_xsec.Get(f"g_xsec_bw_{target}{suffix}")
                g_sys_bw = dir_xsec.Get(f"g_sys_bw_{target}{suffix}")
                if g_xsec_bw and g_sys_bw:
                    g_sys_bw_clone = g_sys_bw.Clone(f"g_sys_bw_clone_{target}{suffix}")
                    g_sys_bw_clone.SetLineColor(color_bw); g_sys_bw_clone.SetFillColorAlpha(color_bw, 0.35)
                    g_sys_bw_clone.SetFillStyle(1001); g_sys_bw_clone.SetMarkerSize(0)

                    g_xsec_bw_clone = g_xsec_bw.Clone(f"g_xsec_bw_clone_{target}{suffix}")
                    g_xsec_bw_clone.SetLineColor(color_bw); g_xsec_bw_clone.SetMarkerColor(color_bw)
                    g_xsec_bw_clone.SetMarkerStyle(ROOT.kFullSquare); g_xsec_bw_clone.SetMarkerSize(1.0)

                    g_sys_bw_clone.Draw("2 SAME"); g_xsec_bw_clone.Draw("P SAME")
                    legend.AddEntry(g_xsec_bw_clone, f"{target} Data (Bin Width)", "pl")

                    for i in range(g_xsec_bw.GetN()):
                        y = g_xsec_bw.GetY()[i]; ey = g_sys_bw.GetErrorY(i)
                        if y > 0:
                            if (y - ey) < y_min and (y - ey) > 0: y_min = y - ey
                            if (y + ey) > y_max: y_max = y + ey
        ...
        lumi_note = ROOT.TLatex(); lumi_note.SetNDC(True); lumi_note.SetTextFont(43); lumi_note.SetTextSize(18)
        ...
        canvas.Update(); canvas.SaveAs(f"cross_section_overlay_Targets{suffix}.pdf")
        dir_overlay = self.get_or_create_dir(self.out_file, "Overlays")
        dir_overlay.cd(); canvas.Write(f"canvas_overlay_Targets_{suffix}")

    def generate_ratio_plot(self, use_true_pt=False, observable="pT"):
        """Calculates and plots the cross-section ratio sigma_pd / (2 * sigma_pp). Evaluates both implementations."""
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        g_xsec_bw_lh2 = dir_lh2.Get(f"g_xsec_bw_LH2{suffix}")
        g_sys_bw_lh2 = dir_lh2.Get(f"g_sys_bw_LH2{suffix}")
        g_xsec_bw_ld2 = dir_ld2.Get(f"g_xsec_bw_LD2{suffix}")
        g_sys_bw_ld2 = dir_ld2.Get(f"g_sys_bw_LD2{suffix}")

        has_bw = observable == "pT2" and bool(g_xsec_bw_lh2) and bool(g_xsec_bw_ld2)
        ...
        if has_bw:
            g_ratio_stat_bw = ROOT.TGraphErrors(); g_ratio_stat_bw.SetName(f"g_ratio_stat_bw{suffix}")
            g_ratio_sys_bw = ROOT.TGraphErrors(); g_ratio_sys_bw.SetName(f"g_ratio_sys_bw{suffix}")

        y_max_ratio, y_min_ratio = -1e9, 1e9

        if observable == "pT2":
            bins = config.PT2_BINS; x_max = 3.5; x_title = "p_{T}^{2} [GeV^{2}]"
        else:
            bins = config.PT_BINS; x_max = 2.0; x_title = "p_{T} [GeV]"

        # Calculate standard (Jacobian) ratio
        pt_idx = 0
        for i_pt in range(len(bins) - 1):
            bin_x = i_pt + 1
            ...
            err_ratio_stat = ratio * math.sqrt((err_stat_ld2/y_ld2)**2 + (err_stat_lh2/y_lh2)**2)
            err_ratio_sys = ratio * math.sqrt((err_sys_ld2/y_ld2)**2 + (err_sys_lh2/y_lh2)**2)

            bin_center = (bins[i_pt] + bins[i_pt+1]) / 2.0
            bin_width = bins[i_pt+1] - bins[i_pt]

            plot_x = bin_center
            if use_true_pt and self.sub_dict_lh2 and self.sub_dict_pd:
                h_pt_lh2 = self.sub_dict_lh2["pt_centroid"]
                h_pt_ld2 = self.sub_dict_pd["pt_centroid"]
                x_lh2 = h_pt_lh2.GetBinContent(bin_x); x_ld2 = h_pt_ld2.GetBinContent(bin_x)
                if observable == "pT2": x_lh2 = x_lh2**2; x_ld2 = x_ld2**2
            ...
            g_ratio_sys.SetPointError(pt_idx, bin_width/2.0, err_ratio_sys)
            pt_idx += 1

            err_max = max(err_ratio_stat, err_ratio_sys)
            if ratio + err_max > y_max_ratio: y_max_ratio = ratio + err_max
            if ratio - err_max < y_min_ratio: y_min_ratio = ratio - err_max

        # Calculate BW ratio
        if has_bw:
            pt_idx_bw = 0
            for i in range(g_xsec_bw_lh2.GetN()):
                x_lh2 = g_xsec_bw_lh2.GetX()[i]; y_lh2 = g_xsec_bw_lh2.GetY()[i]
                y_ld2 = g_xsec_bw_ld2.GetY()[i]
                
                err_stat_lh2 = g_xsec_bw_lh2.GetErrorY(i); err_sys_lh2 = g_sys_bw_lh2.GetErrorY(i)
                err_stat_ld2 = g_xsec_bw_ld2.GetErrorY(i); err_sys_ld2 = g_sys_bw_ld2.GetErrorY(i)
                
                sys_x_err = g_sys_bw_lh2.GetErrorX(i); sys_x = g_sys_bw_lh2.GetX()[i]
                if y_lh2 <= 0 or y_ld2 <= 0: continue
                ...
                # Assume x points are aligned closely enough between LH2/LD2 for the ratio plot x-coordinate
                plot_x_bw = x_lh2 
                g_ratio_stat_bw.SetPoint(pt_idx_bw, plot_x_bw, ratio)
                g_ratio_stat_bw.SetPointError(pt_idx_bw, 0.0, err_ratio_stat)
                
                g_ratio_sys_bw.SetPoint(pt_idx_bw, sys_x, ratio)
                g_ratio_sys_bw.SetPointError(pt_idx_bw, sys_x_err, err_ratio_sys)
                pt_idx_bw += 1

                err_max_bw = max(err_ratio_stat, err_ratio_sys)
                if ratio + err_max_bw > y_max_ratio: y_max_ratio = ratio + err_max_bw
                if ratio - err_max_bw < y_min_ratio: y_min_ratio = ratio - err_max_bw
        ...
        if g_ratio_stat.GetN() > 0:
            ...
            mg = ROOT.TMultiGraph()
            mg.SetTitle(f";{x_title};#sigma_{{pd}} / 2#sigma_{{pp}}")

            # Fit standard ratio
            fit_func = ROOT.TF1("fit_ratio", "pol0", 0.0, x_max)
            g_ratio_sys.Fit(fit_func, "Q0")
            fit_val = fit_func.GetParameter(0); fit_err = fit_func.GetParError(0)

            g_fit_band = ROOT.TGraphErrors()
            g_fit_band.SetPoint(0, 0.0, fit_val); g_fit_band.SetPointError(0, 0.0, fit_err)
            g_fit_band.SetPoint(1, x_max, fit_val); g_fit_band.SetPointError(1, 0.0, fit_err)
            g_fit_band.SetFillColorAlpha(ROOT.kPink, 0.4); g_fit_band.SetFillStyle(1001)

            if fit_val + fit_err > y_max_ratio: y_max_ratio = fit_val + fit_err
            if fit_val - fit_err < y_min_ratio: y_min_ratio = fit_val - fit_err

            # Fit BW ratio if present
            if has_bw:
                fit_func_bw = ROOT.TF1("fit_ratio_bw", "pol0", 0.0, x_max)
                g_ratio_sys_bw.Fit(fit_func_bw, "Q0")
                fit_val_bw = fit_func_bw.GetParameter(0); fit_err_bw = fit_func_bw.GetParError(0)
                
                g_fit_band_bw = ROOT.TGraphErrors()
                g_fit_band_bw.SetPoint(0, 0.0, fit_val_bw); g_fit_band_bw.SetPointError(0, 0.0, fit_err_bw)
                g_fit_band_bw.SetPoint(1, x_max, fit_val_bw); g_fit_band_bw.SetPointError(1, 0.0, fit_err_bw)
                g_fit_band_bw.SetFillColorAlpha(ROOT.kAzure+1, 0.3); g_fit_band_bw.SetFillStyle(1001)

                if fit_val_bw + fit_err_bw > y_max_ratio: y_max_ratio = fit_val_bw + fit_err_bw
                if fit_val_bw - fit_err_bw < y_min_ratio: y_min_ratio = fit_val_bw - fit_err_bw
            ...
            leg = ROOT.TLegend(0.40 if has_bw else 0.65, 0.70, 0.88, 0.88)
            leg.SetBorderSize(0)

            # Draw standard ratio
            mg.Add(g_fit_band, "3")
            g_ratio_sys.SetMarkerSize(0)
            g_ratio_sys.SetLineColor(ROOT.kGreen+2); g_ratio_sys.SetFillColorAlpha(ROOT.kGreen-9, 0.5); g_ratio_sys.SetFillStyle(1001)
            mg.Add(g_ratio_sys, "2")
            leg.AddEntry(g_ratio_sys, "Sys. Unc. (Jacobian)", "f")

            g_ratio_stat.SetMarkerStyle(ROOT.kFullCircle); g_ratio_stat.SetMarkerColor(ROOT.kBlack); g_ratio_stat.SetLineColor(ROOT.kBlack)
            mg.Add(g_ratio_stat, "P")
            leg.AddEntry(g_ratio_stat, "Ratio Data (Jacobian)", "lep")

            # Draw BW ratio
            if has_bw:
                mg.Add(g_fit_band_bw, "3")
                g_ratio_sys_bw.SetMarkerSize(0)
                g_ratio_sys_bw.SetLineColor(ROOT.kBlue); g_ratio_sys_bw.SetFillColorAlpha(ROOT.kAzure-9, 0.5); g_ratio_sys_bw.SetFillStyle(1001)
                mg.Add(g_ratio_sys_bw, "2")
                leg.AddEntry(g_ratio_sys_bw, "Sys. Unc. (Bin Width)", "f")
                
                g_ratio_stat_bw.SetMarkerStyle(ROOT.kFullSquare); g_ratio_stat_bw.SetMarkerColor(ROOT.kBlue); g_ratio_stat_bw.SetLineColor(ROOT.kBlue)
                mg.Add(g_ratio_stat_bw, "P")
                leg.AddEntry(g_ratio_stat_bw, "Ratio Data (Bin Width)", "lep")

            mg.Draw("A"); mg.GetXaxis().CenterTitle(); mg.GetYaxis().CenterTitle()
            mg.GetXaxis().SetLimits(0.0, x_max)
            ...
            if has_bw:
                fit_func_bw.SetLineColor(ROOT.kBlue); fit_func_bw.SetLineWidth(2); fit_func_bw.Draw("SAME")

            leg.Draw()

            latex_fit = ROOT.TLatex(); latex_fit.SetTextFont(42); latex_fit.SetTextSize(0.035)
            y_range_actual = mg.GetYaxis().GetXmax() - mg.GetYaxis().GetXmin()
            
            y_text = fit_val + fit_err + (y_range_actual * 0.05)
            latex_fit.SetTextColor(ROOT.kRed)
            latex_fit.DrawLatex(0.1 * x_max, y_text, f"Jacobian Fit: {fit_val:.4f} #pm {fit_err:.4f}")

            if has_bw:
                y_text_bw = fit_val_bw - fit_err_bw - (y_range_actual * 0.05)
                latex_fit.SetTextColor(ROOT.kBlue)
                latex_fit.DrawLatex(0.1 * x_max, y_text_bw, f"Bin Width Fit: {fit_val_bw:.4f} #pm {fit_err_bw:.4f}")

            prelim = ROOT.TLatex(); prelim.SetNDC(True); prelim.SetTextColor(ROOT.kBlue)
            prelim.SetTextAlign(33); prelim.SetTextSize(0.05)
            prelim.DrawLatex(0.85, 0.55, "Preliminary")

            c_ratio.SaveAs(f"CrossSection_Ratio_pd_2pp{suffix}.pdf")

            dir_ratio.cd()
            g_ratio_stat.Write(); g_ratio_sys.Write()
            if has_bw: g_ratio_stat_bw.Write(); g_ratio_sys_bw.Write()
            c_ratio.Write(); c_ratio.Close()

    def generate_combined_ratio_overlay_plot(self, use_true_pt=False, observable="pT"):
        """Creates a split canvas with cross-section overlays on top and the ratio plot on bottom. Refactored specifically to output ONLY BW components for pT2 plots."""
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        g_xsec_lh2 = dir_lh2.Get(f"g_xsec_LH2{suffix}"); g_sys_lh2 = dir_lh2.Get(f"g_sys_LH2{suffix}")
        g_xsec_ld2 = dir_ld2.Get(f"g_xsec_LD2{suffix}"); g_sys_ld2 = dir_ld2.Get(f"g_sys_LD2{suffix}")
        g_ratio_stat = dir_ratio.Get(f"g_ratio_stat{suffix}"); g_ratio_sys = dir_ratio.Get(f"g_ratio_sys{suffix}")

        has_bw = observable == "pT2"
        g_xsec_bw_lh2, g_sys_bw_lh2, g_xsec_bw_ld2, g_sys_bw_ld2 = None, None, None, None
        g_ratio_stat_bw, g_ratio_sys_bw = None, None

        if has_bw:
            g_xsec_bw_lh2 = dir_lh2.Get(f"g_xsec_bw_LH2{suffix}"); g_sys_bw_lh2 = dir_lh2.Get(f"g_sys_bw_LH2{suffix}")
            g_xsec_bw_ld2 = dir_ld2.Get(f"g_xsec_bw_LD2{suffix}"); g_sys_bw_ld2 = dir_ld2.Get(f"g_sys_bw_LD2{suffix}")
            g_ratio_stat_bw = dir_ratio.Get(f"g_ratio_stat_bw{suffix}"); g_ratio_sys_bw = dir_ratio.Get(f"g_ratio_sys_bw{suffix}")
            has_bw = bool(g_xsec_bw_lh2 and g_xsec_bw_ld2 and g_ratio_stat_bw)

        canvas = ROOT.TCanvas(f"c_combined_{suffix}", "Combined Overlay and Ratio", 800, 800)

        # --- TPad 1: Cross Sections Overlay (Top 65%) ---
        ...
        if has_bw:
            # LH2 BW (Azure)
            g_sys_bw_lh2_clone = g_sys_bw_lh2.Clone(); g_sys_bw_lh2_clone.SetLineColor(ROOT.kAzure+1); g_sys_bw_lh2_clone.SetFillColorAlpha(ROOT.kAzure+1, 0.3)
            g_xsec_bw_lh2_clone = g_xsec_bw_lh2.Clone(); g_xsec_bw_lh2_clone.SetLineColor(ROOT.kAzure+1); g_xsec_bw_lh2_clone.SetMarkerColor(ROOT.kAzure+1); g_xsec_bw_lh2_clone.SetMarkerStyle(ROOT.kFullSquare)

            # LD2 BW (Magenta)
            g_sys_bw_ld2_clone = g_sys_bw_ld2.Clone(); g_sys_bw_ld2_clone.SetLineColor(ROOT.kMagenta); g_sys_bw_ld2_clone.SetFillColorAlpha(ROOT.kMagenta, 0.3)
            g_xsec_bw_ld2_clone = g_xsec_bw_ld2.Clone(); g_xsec_bw_ld2_clone.SetLineColor(ROOT.kMagenta); g_xsec_bw_ld2_clone.SetMarkerColor(ROOT.kMagenta); g_xsec_bw_ld2_clone.SetMarkerStyle(ROOT.kFullSquare)

            mg_top.Add(g_sys_bw_lh2_clone, "2"); mg_top.Add(g_sys_bw_ld2_clone, "2")
            mg_top.Add(g_xsec_bw_lh2_clone, "P"); mg_top.Add(g_xsec_bw_ld2_clone, "P")
        else:
            # Fallback functionality for generic variables that lack BW graphs
            g_sys_lh2_clone = g_sys_lh2.Clone(); g_sys_lh2_clone.SetLineColor(ROOT.kRed); g_sys_lh2_clone.SetFillColorAlpha(ROOT.kPink - 9, 0.5)
            g_xsec_lh2_clone = g_xsec_lh2.Clone(); g_xsec_lh2_clone.SetLineColor(ROOT.kRed); g_xsec_lh2_clone.SetMarkerColor(ROOT.kRed); g_xsec_lh2_clone.SetMarkerStyle(ROOT.kFullCircle)

            g_sys_ld2_clone = g_sys_ld2.Clone(); g_sys_ld2_clone.SetLineColor(ROOT.kBlue); g_sys_ld2_clone.SetFillColorAlpha(ROOT.kAzure + 1, 0.5) 
            g_xsec_ld2_clone = g_xsec_ld2.Clone(); g_xsec_ld2_clone.SetLineColor(ROOT.kBlue); g_xsec_ld2_clone.SetMarkerColor(ROOT.kBlue); g_xsec_ld2_clone.SetMarkerStyle(ROOT.kFullCircle)

            mg_top.Add(g_sys_lh2_clone, "2"); mg_top.Add(g_sys_ld2_clone, "2")
            mg_top.Add(g_xsec_lh2_clone, "P"); mg_top.Add(g_xsec_ld2_clone, "P")

        mg_top.Draw("A"); mg_top.SetTitle("")

        if observable == "pT2":
            x_max = 3.5; x_title = "p_{T}^{2} [GeV^{2}]"; y_title = "d#sigma/dp_{T}^{2} [nb/GeV^{2}]"
        else:
            x_max = 2.0; x_title = "p_{T} [GeV]"; y_title = "d#sigma/dp_{T} [nb/GeV]"

        mg_top.GetXaxis().SetLimits(0.0, x_max)
        mg_top.GetYaxis().SetTitle(y_title); mg_top.GetYaxis().CenterTitle()
        mg_top.GetYaxis().SetTitleFont(43); mg_top.GetYaxis().SetTitleSize(22); mg_top.GetYaxis().SetTitleOffset(2.0)
        mg_top.GetYaxis().SetLabelFont(43); mg_top.GetYaxis().SetLabelSize(20)

        mg_top.GetXaxis().SetTitle(x_title); mg_top.GetXaxis().CenterTitle()
        mg_top.GetXaxis().SetTitleFont(43); mg_top.GetXaxis().SetTitleSize(22); mg_top.GetXaxis().SetTitleOffset(1.2) 
        mg_top.GetXaxis().SetLabelFont(43); mg_top.GetXaxis().SetLabelSize(20)

        y_max_val = 0
        graphs_to_check = [g_xsec_bw_lh2, g_xsec_bw_ld2] if has_bw else [g_xsec_lh2, g_xsec_ld2]
        err_graphs = [g_sys_bw_lh2, g_sys_bw_ld2] if has_bw else [g_sys_lh2, g_sys_ld2]
        
        for g, g_e in zip(graphs_to_check, err_graphs):
            for i in range(g.GetN()):
                val = g.GetY()[i] + g_e.GetErrorY(i)
                if val > y_max_val: y_max_val = val
                
        mg_top.SetMinimum(0.0); mg_top.SetMaximum(y_max_val * 1.4)

        leg_top = ROOT.TLegend(0.40 if has_bw else 0.65, 0.65, 0.88, 0.88)
        leg_top.SetBorderSize(0); leg_top.SetFillStyle(0); leg_top.SetTextFont(43); leg_top.SetTextSize(15)
        if has_bw:
            leg_top.AddEntry(g_xsec_bw_lh2_clone, "LH2 Data", "pl"); leg_top.AddEntry(g_sys_bw_lh2_clone, "LH2 Sys. Unc.", "f")
            leg_top.AddEntry(g_xsec_bw_ld2_clone, "LD2 Data", "pl"); leg_top.AddEntry(g_sys_bw_ld2_clone, "LD2 Sys. Unc.", "f")
        else:
            leg_top.AddEntry(g_xsec_lh2_clone, "LH2 Data", "pl"); leg_top.AddEntry(g_sys_lh2_clone, "LH2 Sys. Unc.", "f")
            leg_top.AddEntry(g_xsec_ld2_clone, "LD2 Data", "pl"); leg_top.AddEntry(g_sys_ld2_clone, "LD2 Sys. Unc.", "f")
        leg_top.Draw()
        ...
        if has_bw:
            fit_func_bw = ROOT.TF1("fit_ratio_comb_bw", "pol0", 0.0, x_max)
            g_ratio_sys_bw.Fit(fit_func_bw, "Q0")
            fit_val_bw = fit_func_bw.GetParameter(0); fit_err_bw = fit_func_bw.GetParError(0)
            ...
            g_ratio_sys_bw_clone = g_ratio_sys_bw.Clone(); g_ratio_sys_bw_clone.SetLineColor(ROOT.kBlue); g_ratio_sys_bw_clone.SetFillColorAlpha(ROOT.kAzure-9, 0.5)
            g_ratio_stat_bw_clone = g_ratio_stat_bw.Clone(); g_ratio_stat_bw_clone.SetMarkerStyle(ROOT.kFullSquare); g_ratio_stat_bw_clone.SetMarkerColor(ROOT.kBlue); g_ratio_stat_bw_clone.SetLineColor(ROOT.kBlue)

            mg_bottom.Add(g_fit_band_bw, "3"); mg_bottom.Add(g_ratio_sys_bw_clone, "2"); mg_bottom.Add(g_ratio_stat_bw_clone, "P")

            if fit_val_bw + fit_err_bw > y_max_r: y_max_r = fit_val_bw + fit_err_bw
            if fit_val_bw - fit_err_bw < y_min_r: y_min_r = fit_val_bw - fit_err_bw
            for i in range(g_ratio_stat_bw.GetN()):
                y = g_ratio_stat_bw.GetY()[i]
                err = max(g_ratio_stat_bw.GetErrorY(i), g_ratio_sys_bw.GetErrorY(i))
                if y + err > y_max_r: y_max_r = y + err
                if y - err < y_min_r: y_min_r = y - err
        else:
            fit_func = ROOT.TF1("fit_ratio_comb", "pol0", 0.0, x_max)
            g_ratio_sys.Fit(fit_func, "Q0")
            fit_val = fit_func.GetParameter(0); fit_err = fit_func.GetParError(0)
            ...
            g_ratio_sys_clone = g_ratio_sys.Clone(); g_ratio_sys_clone.SetLineColor(ROOT.kGreen+2); g_ratio_sys_clone.SetFillColorAlpha(ROOT.kGreen-9, 0.5)
            g_ratio_stat_clone = g_ratio_stat.Clone(); g_ratio_stat_clone.SetMarkerStyle(ROOT.kFullCircle)

            mg_bottom.Add(g_fit_band, "3"); mg_bottom.Add(g_ratio_sys_clone, "2"); mg_bottom.Add(g_ratio_stat_clone, "P")
        ...
        mg_bottom.Draw("A"); mg_bottom.SetTitle("")
        mg_bottom.GetXaxis().SetLimits(0.0, x_max)
        mg_bottom.GetYaxis().SetTitle("#sigma_{pd}/2#sigma_{pp}"); mg_bottom.GetYaxis().CenterTitle()
        mg_bottom.GetYaxis().SetTitleFont(43); mg_bottom.GetYaxis().SetTitleSize(22); mg_bottom.GetYaxis().SetTitleOffset(2.0)
        mg_bottom.GetYaxis().SetLabelFont(43); mg_bottom.GetYaxis().SetLabelSize(20); mg_bottom.GetYaxis().SetNdivisions(505) 
        mg_bottom.GetXaxis().SetTitle(x_title); mg_bottom.GetXaxis().CenterTitle()
        mg_bottom.GetXaxis().SetTitleFont(43); mg_bottom.GetXaxis().SetTitleSize(22); mg_bottom.GetXaxis().SetTitleOffset(1.2) 
        ...
        latex_fit = ROOT.TLatex(); latex_fit.SetTextFont(43); latex_fit.SetTextSize(20)

        if has_bw:
            fit_func_bw.SetLineColor(ROOT.kBlue); fit_func_bw.SetLineWidth(2); fit_func_bw.Draw("SAME")
            y_text_bw = fit_val_bw - fit_err_bw - ((mg_bottom.GetYaxis().GetXmax() - mg_bottom.GetYaxis().GetXmin()) * 0.05)
            latex_fit.SetTextColor(ROOT.kBlue)
            latex_fit.DrawLatex(0.1 * x_max, y_text_bw*1.15, f"Bin Width Fit: {fit_val_bw:.4f} #pm {fit_err_bw:.4f}")
        else:
            fit_func.SetLineColor(ROOT.kRed); fit_func.SetLineWidth(2); fit_func.Draw("SAME")
        ...
        canvas.SaveAs(f"Combined_XSec_Ratio{suffix}.pdf")
        dir_comb = self.get_or_create_dir(self.out_file, "Combined_Plots")
        dir_comb.cd(); canvas.Write(f"c_combined_ratio_overlay{suffix}"); canvas.Close()
    ...
    def calculate_cross_sections(self):
        ...
        for observable in ["pT", "pT2"]:
            for use_true_pt in [False, True]:
                if self.sub_dict_lh2:
                    self.calculate_and_plot_cross_section(self.sub_dict_lh2, "LH2", config.GLOBAL_CONSTANT_LH2, use_true_pt, observable)
                if self.sub_dict_pd:
                    self.calculate_and_plot_cross_section(self.sub_dict_pd, "LD2", config.GLOBAL_CONSTANT_LD2, use_true_pt, observable)
                    
                self.generate_overlay_plot(use_true_pt, observable)
                self.generate_ratio_plot(use_true_pt, observable)
                self.generate_combined_ratio_overlay_plot(use_true_pt, observable) 
