...
# Note: While named MOL_CM2 / MOL_CM3, the numerical values functionally represent 
# mass densities (g/cm^2 and g/cm^3) for the purpose of the beam attenuation math below.
...
def print_physics_constants():
    ...
    table.add_row("LH2 Target Density (g/cm²)", f"{LH2_TARGET_DENSITY_MOL_CM2:.4f}")
    table.add_row("LD2 Target Density (g/cm²)", f"{LD2_TARGET_DENSITY_MOL_CM2:.4f}")
    table.add_row("LH2 Target Length (cm)", f"{LH2_TARGET_LENGTH_CM:.1f}")
    table.add_row("LD2 Target Length (cm)", f"{LD2_TARGET_LENGTH_CM:.1f}")
    table.add_row("LH2 Target Density (g/cm³)", f"{LH2_TARGET_DENSITY_MOL_CM3:.4f}")
    table.add_row("LD2 Target Density (g/cm³)", f"{LD2_TARGET_DENSITY_MOL_CM3:.4f}")
    ...
    # Bins 
    pt_str = np.array2string(PT_BINS, precision=2, separator=', ')
    pt2_str = np.array2string(PT2_BINS, precision=4, separator=', ')
    table.add_row("pT Bins Definition", pt_str)
    table.add_row("pT² Bins Definition", pt2_str)
