...
class DYCrossSectionAnalyzer:
    ...
    def __init__(self, lh2_files, ld2_files, flask_files, out_filename="All_XSec_Objects.root"):
        ...
        # Load dynamic bin-by-bin roadset systematics AND weighted means for pT2
        lh2_sys_csv = "/root/github/e906-development/src/xsec_pT_squard/RS57-70_weighted_average/Roadset_Sys_StdDev_LH2_geom_pT2.csv"
        ld2_sys_csv = "/root/github/e906-development/src/xsec_pT_squard/RS57-70_weighted_average/Roadset_Sys_StdDev_LD2_geom_pT2.csv"
    ...
    def calculate_and_plot_cross_section(self, h_sub_dict, target_label, global_constant, use_true_pt=False, observable="pT2"):
        """Builds single differential cross-sections vs pT^2, incorporating weighted CSV data."""
        ...
        n_pt_bins = len(config.PT2_BINS) - 1
...
""" % (target_label, target_label)
        ...
        h1_xsec = ROOT.TH1D(f"h1_xsec_{target_label}{suffix}", f"Single Differential Cross Section {target_label};{x_title};{y_title}", n_pt_bins, config.PT2_BINS)
        h1_sys = ROOT.TH1D(f"h1_sys_{target_label}{suffix}", f"Systematic Error {target_label};{x_title};{y_title}", n_pt_bins, config.PT2_BINS)
        ...
        for i_pt in range(n_pt_bins):
            ...
            if actual_pt_center <= 0: actual_pt_center = (pt_min + pt_max) / 2.0
            ...
            denominator = pt2_width * acceptance
            raw_dsigma_dpt2 = numerator / denominator
            
            stat_unc_raw = (Y_sub_stat_err / Y_sub) * raw_dsigma_dpt2
            sys_tot_yield_raw = (Y_sub_sys_err / Y_sub) * raw_dsigma_dpt2
            sys_psip_cont_raw = psip_ratio * raw_dsigma_dpt2
            ...
            if psip_ratio > 0.0 and not use_true_pt:
                ...
                s_sigma_psip_col = f"{sum_xsec:.4e} $\\pm$ {stat_unc:.4e} $\\pm$ {sys_unc_bin:.4e}"
                latex_psip_table_content += f"{s_pt} & {s_mass} & {s_ratio} & {s_sigma_psip_col} & {sys_psip_cont:.4e} \\\\ \n\\hline\n"
            ...
            if sum_xsec > 0:
                ...
                # Note: Global 10% luminosity error is explicitly omitted mathematically
                ...
                g_sys.SetPoint(point_idx, plot_x_geom, sum_xsec)
                g_sys.SetPointError(point_idx, pt2_width/2.0, total_sys_err)
                
                g_xsec.SetPoint(point_idx, plot_x, sum_xsec)
                g_xsec.SetPointError(point_idx, pt2_width/2.0, total_stat_err) 
        ...
        if g_xsec.GetN() > 0:
            ...
            mg.GetXaxis().SetLimits(0.0, 3.5); mg.GetXaxis().SetTitleOffset(1.3); mg.GetYaxis().SetTitleOffset(1.7)
    ...
    def generate_overlay_plot(self, use_true_pt=False, observable="pT2"):
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        h_frame = canvas.DrawFrame(0.0, 0.0, 3.5, 1.0)
        h_frame.SetTitle("DY Absolute Cross-Section Vs p_{T}^{2}")
        h_frame.GetXaxis().SetTitle("p_{T}^{2} [GeV^{2}]"); h_frame.GetXaxis().CenterTitle()
        h_frame.GetXaxis().SetTitleOffset(1.3); h_frame.GetYaxis().SetTitle("d#sigma/dp_{T}^{2} [nb/GeV^{2}]")
        ...
        line_base = ROOT.TLine(0.0, baseline, 3.5, baseline)
    ...
    def generate_ratio_plot(self, use_true_pt=False, observable="pT2"):
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        for i_pt in range(len(config.PT2_BINS) - 1):
            bin_x = i_pt + 1
            ...
            pt_min = config.PT2_BINS[i_pt]
            pt_max = config.PT2_BINS[i_pt+1]
            ...
            bin_center = (config.PT2_BINS[i_pt] + config.PT2_BINS[i_pt+1]) / 2.0

            if use_true_pt and h_pt_lh2 and h_pt_ld2:
                x_lh2 = h_pt_lh2.GetBinContent(bin_x)
                x_ld2 = h_pt_ld2.GetBinContent(bin_x)
                plot_x = (x_lh2**2 + x_ld2**2) / 2.0
            else:
                plot_x = bin_center

            pt_width = config.PT2_BINS[i_pt+1] - config.PT2_BINS[i_pt]

            # Replaced 0.0 with pt_width/2.0 to draw horizontal error bars
            g_ratio_stat.SetPoint(pt_idx, plot_x, ratio)
            g_ratio_stat.SetPointError(pt_idx, pt_width/2.0, err_ratio_stat)

            # Center systematic band at ratio value
        ...
        if use_true_pt:
            csv_filename = "pd_2pp_errors_pT2.csv"
            with open(csv_filename, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["pT2 Bin [GeV2]", "Ratio", "Stat Err", "Sys Err", "Total Err"])
        ...
        if g_ratio_total.GetN() > 0:
            ...
            mg = ROOT.TMultiGraph()
            mg.SetTitle(";p_{T}^{2} [GeV^{2}];#sigma_{pd} / 2#sigma_{pp}")

            fit_func = ROOT.TF1("fit_ratio", "pol0", 0.0, 3.5)
            ...
            g_fit_band.SetPoint(1, 3.5, fit_val)
            ...
            # Green Band is hidden (commented out)
            ...
            mg.Draw("A"); mg.GetXaxis().CenterTitle(); mg.GetYaxis().CenterTitle()
            mg.GetXaxis().SetLimits(0.0, 3.5)
            mg.GetXaxis().SetTitleOffset(1.3); mg.GetYaxis().SetTitleOffset(1.3)

            line1 = ROOT.TLine(0.0, 1.0, 3.5, 1.0)
            line1.SetLineStyle(2); line1.SetLineColor(ROOT.kGray+2); line1.SetLineWidth(2)
            line1.Draw("SAME")
            
            line0 = ROOT.TLine(0.0, 0.0, 3.5, 0.0)
            line0.SetLineStyle(2); line0.SetLineColor(ROOT.kGray+1); line0.SetLineWidth(1)
            line0.Draw("SAME")
            ...
            latex_fit.DrawLatex(0.65, 1.25, f"Best Fit: {fit_val:.4f} #pm {fit_err:.4f}")
    ...
    def generate_combined_ratio_overlay_plot(self, use_true_pt=False, observable="pT2"):
        suffix = f"_true_pt_{observable}" if use_true_pt else f"_geom_{observable}"
        ...
        mg_top.Draw("A")
        mg_top.SetTitle("")
        mg_top.GetXaxis().SetLimits(0.0, 3.5)

        mg_top.GetYaxis().SetTitle("d#sigma/dp_{T}^{2} [nb/GeV^{2}]")
        ...
        mg_top.GetXaxis().SetTitle("p_{T}^{2} [GeV^{2}]")
        ...
        line_zero = ROOT.TLine(0.0, 0.0, 3.5, 0.0)
        line_zero.SetLineStyle(1); line_zero.SetLineColor(ROOT.kBlack)
        line_zero.Draw("SAME")

        leg_top = ROOT.TLegend(0.45, 0.65, 0.88, 0.88)
        ...
        prelim.SetTextSize(0.05)
        prelim.DrawLatex(0.35, 0.79, "Preliminary")
        
        prelim.SetTextSize(0.0252) 
        prelim.DrawLatex(0.35, 0.73, "Run Period 2014-2015")
        ...
        fit_func = ROOT.TF1("fit_ratio_comb", "pol0", 0.0, 3.5)
        g_ratio_total.Fit(fit_func, "Q0")
        fit_val = fit_func.GetParameter(0)
        fit_err = fit_func.GetParError(0)
        ...
        g_fit_band.SetPoint(1, 3.5, fit_val); g_fit_band.SetPointError(1, 0.0, fit_err)
        ...
        # g_ratio_sys_clone.SetMarkerSize(0)
        # g_ratio_sys_clone.SetLineColor(ROOT.kGreen+2)
        # g_ratio_sys_clone.SetFillColorAlpha(ROOT.kGreen-9, 0.5)
        # g_ratio_sys_clone.SetFillStyle(1001)
        # mg_bottom.Add(g_ratio_sys_clone, "2")
        
        mg_bottom.Add(g_ratio_total_clone, "P")

        # Enforcing exact dynamic Y-axis limits requested
        mg_bottom.SetMinimum(1.0)
        mg_bottom.SetMaximum(1.3)

        mg_bottom.Draw("A")
        mg_bottom.SetTitle("")
        mg_bottom.GetXaxis().SetLimits(0.0, 3.5)
        ...
        mg_bottom.GetXaxis().SetTitle("p_{T}^{2} [GeV^{2}]")
    ...
    def calculate_cross_sections(self):
        ...
        for use_true_pt in [False, True]:
            if self.sub_dict_lh2:
                self.calculate_and_plot_cross_section(self.sub_dict_lh2, "LH2", config.GLOBAL_CONSTANT_LH2, use_true_pt, "pT2")
            if self.sub_dict_pd:
                self.calculate_and_plot_cross_section(self.sub_dict_pd, "LD2", config.GLOBAL_CONSTANT_LD2, use_true_pt, "pT2")
                
            self.generate_overlay_plot(use_true_pt, "pT2")
            self.generate_ratio_plot(use_true_pt, "pT2")
            self.generate_combined_ratio_overlay_plot(use_true_pt, "pT2") 
