...
# Input Files
# ==========================================
input_root_file = "merged_RS67_3089_LD2_recoeff.root" # Update if needed
...
def generate_recoeff_histogram():
    ...
    print("Loading NPZ file to recreate interpolation bins...")
    npz_data = np.load(input_npz_file)
    x_curve = npz_data['x']
    
    # Recreate loc_data (the interpolation bin index for each event)
    loc_data_tot = np.digitize(data_tot['D1'], x_curve) - 1

    # Create TH2D
    h_reco = ROOT.TH2D("E_total_reco_LD2", "Avg Reco Eff with Correlated Errors (LD2);Mass [GeV];x_{F}", 
    ...
    for i_m in range(len(mass_bins_np) - 1):
        ...
        for i_x in range(len(xf_bins_np) - 1):
            ...
            eff_arr = data_tot["recoeff"][mask]
            err_arr = data_tot["recoeff_error"][mask]
            loc_arr = loc_data_tot[mask]
    ...
    # Save to PDF
    out_pdf = "E_total_reco_LD2_correlated.pdf"
    print(f"Drawing and saving to {out_pdf}...")
    
    c = ROOT.TCanvas("c_reco", "", 1200, 900)
