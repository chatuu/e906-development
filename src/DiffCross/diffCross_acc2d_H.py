...
ROOT_FILE_PATH = "/root/github/e906-development/res/ROOTFiles/MixedEvents/merged_RS67_3089LH2.root"
#MIX_FILE_NAME_PATH = "/root/github/e906-development/res/ROOTFiles/Shivangi/mixFPGA4_67_R008_preBin2.root"
MIX_FILE_NAME_PATH = "/root/github/e906-development/res/ROOTFiles/MixedEvents/merged_RS67_3089LH2.root"
Flask_FILE_NAME_PATH = "/root/github/e906-development/res/ROOTFiles/MixedEvents/merged_RS67_3089flask.root"
Flask_MIX_FILE_NAME_PATH = "/root/github/e906-development/res/ROOTFiles/MixedEvents/merged_RS67_3089flask.root"
...
def diffCross_acc2d_H(binNum):
    ...
    file_paths_to_load = {
        ...
        "emptyFlaskTree": Flask_FILE_NAME_PATH,
        "mixFlaskTree": Flask_MIX_FILE_NAME_PATH,
    ...
    for tree_name, path in file_paths_to_load.items():
        try:
            ...
            if tree_name == "dataTree" or tree_name=="emptyFlaskTree":
                tree = file_obj.Get("result")
            elif tree_name == "mixTree" or tree_name=="mixFlaskTree":
                tree = file_obj.Get("result_mix")
            else:
                tree = file_obj.Get("Tree")           
        
                # For data and empty Flask trees, we use the ROOT_FILE_PATH
                # which is already set to the main ROOT file.
    ...
    emptyFlaskTree = tree_dict.get("emptyFlaskTree")
    mixFlaskTree = tree_dict.get("mixFlaskTree")
    jpTree = tree_dict.get("jpTree")
    ppTree = tree_dict.get("ppTree")

    if not all([dataTree, mixTree, emptyFlaskTree, mixFlaskTree, jpTree, ppTree]):
    ...
    hDataF.Sumw2()
    hDataFMix = ROOT.TH1F("hDataFMix" + pid_suffix, "Data Flask Mix", NBINS_MASS, EDGES_MASS_ARR)
    hDataFMix.Sumw2()
    ...
    cut_data_flask_mix = f"mass^3*(({cuts.gmcTemp_charm}) && ({cuts.physics}) && ({xFCut})  && ({cuts.occ}))"
    ...
    try:
        ...
        emptyFlaskTree.Draw(f"mass>>hDataF{pid_suffix}", cut_data_flask, "goff")
        mixFlaskTree.Draw(f"mass>>hDataF{pid_suffix}", cut_data_flask_mix, "goff")
    ...
    hData.Add(hDataF, -1)  # Subtract Flask data from main data
    hData.Add(hDataFMix, -1)  # Subtract Flask mixed data from main data
