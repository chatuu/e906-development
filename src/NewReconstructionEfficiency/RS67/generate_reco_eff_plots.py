...
def get_correlated_mean_and_error(eff_arr, err_arr, loc_arr):
    """
    Calculates the mean efficiency and its correlated uncertainty 
    using the covariance matrix method for a specific bin.
    """
    N = len(eff_arr)
    if N == 0: 
        return 0.0, 0.0
    ...
    # Generate Covariance Matrix for this bin
    diff_matrix = np.abs(loc_arr[:, None] - loc_arr[None, :])
    correl_matrix = np.zeros((N, N))
    correl_matrix[diff_matrix == 0] = 1.0  # Shared both interpolation points
    correl_matrix[diff_matrix == 1] = 1.0  # Shared one interpolation point
...
def add_latex_to_bin(hist, x_center, y_center, value, error):
    val_f = float(value)
    err_f = float(error)
    if np.isnan(val_f) or np.isinf(val_f) or val_f == 0.0: return
...
def generate_recoeff_histogram(input_root_file, input_npz_file, target_name, tree_name, output_root_file):
    ...
    try:
        ...
        if tree_name not in f_in:
            print(f"Error: Tree '{tree_name}' not found in {input_root_file}")
            return
        data = e906_chuck_cuts(f_in[tree_name])
    except Exception as e:
        print(f"Error reading root file: {e}")
        return

    print(f"Loading NPZ file ({input_npz_file}) to recreate interpolation bins...")
    npz_data = np.load(input_npz_file)
    x_curve = npz_data['x']
    
    # Recreate loc_data (the interpolation bin index for each event)
    # Handle D1 depending on the tree
    if tree_name == 'result_mix':
        # Need to calculate D1 the same way the earlier script did for mix
        if 'ptrk_D1' in data and 'ntrk_D1' in data:
            d1_values = 0.5 * (data['ptrk_D1'] + data['ntrk_D1'])
        else:
            d1_values = data['D1']
    else:
        d1_values = data['D1']
        
    loc_data = np.digitize(d1_values, x_curve) - 1
    ...
    # Create TH2D
    h_reco = ROOT.TH2D(hist_name, hist_title, 
    ...
    print("Calculating correlated errors per bin...")
    for i_m in range(len(mass_bins_np) - 1):
        ...
        for i_x in range(len(xf_bins_np) - 1):
            ...
            # Mask for this specific bin
            mask = (data["mass"] >= m_low) & (data["mass"] < m_high) & \
                   (data["xF"] >= x_low) & (data["xF"] < x_high)
            
            # Extract arrays for the bin
            eff_arr = data["recoeff"][mask]
            err_arr = data["recoeff_error"][mask]
            loc_arr = loc_data[mask]
            
            # Calculate mean and correlated error
