...
def calculate_roadset_variance(target="LH2", suffix="_geom", use_equal_weights=True):
    """
    ...
    for bin_idx in sorted(cross_sections.keys()):
        ...
        print(f"{bin_idx:<5} | {mean_val:<12.6f} | {std_dev:<12.6f} | {rel_err:.2f}%")
...
if __name__ == "__main__":
    calculate_roadset_variance("LH2", "_geom", use_equal_weights=False)
    calculate_roadset_variance("LD2", "_geom", use_equal_weights=False)