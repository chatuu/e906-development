...
class DYCrossSectionAnalyzer:
    ...
    def __init__(self, lh2_files, ld2_files, flask_files, out_filename="All_XSec_Objects.root"):
        ...
        # Unique prefix so PDFs aren't overwritten when ran in a loop over roadsets
        self.run_label = out_filename.replace("XSec_", "").replace("_Objects.root", "")
    ...
    def save_1d_pdfs(self, hists_dict, label):
        ...
        for name, hist in hists_dict.items():
            ...
            c.SaveAs(f"{self.run_label}_{name}_{label}.pdf")
            c.Close()
    ...
    def generate_efficiency_csv(self, data_tot, loc_tot, m_low, m_high, pt_low, pt_high, pt_idx, target_label):
        ...
        filename = f"{self.run_label}_reco_pT_{pt_idx}_{target_label}.csv"
    ...
    def process_kinematics(self):
        ...
        csv_filename_lh2 = f"{self.run_label}_Table_Kinematics_LH2_vs_pT.csv"
        csv_filename_ld2 = f"{self.run_label}_Table_Kinematics_LD2_vs_pT.csv"
    ...
    def generate_subtracted_plot(self, hists_target, hists_flask, flask_norm, target_label):
        ...
        c.SaveAs(f"{self.run_label}_{name}.pdf"); c.Close()
...
""" % (target_label, target_label)
        ...
        if g_xsec.GetN() > 0:
            ...
            if target_label == "LH2": road_sys = "4.4%"
            elif target_label == "LD2": road_sys = "5.4%"
            else: road_sys = "0.0%"
            ...
            c_xsec.SaveAs(f"{self.run_label}_CrossSection_{target_label}{suffix}_vs_pT.pdf")
            
            dir_xsec.cd()
            g_xsec.Write(); g_sys.Write(); h1_xsec.Write(); h1_sys.Write(); c_xsec.Write(); c_xsec.Close()

        if not use_true_pt:
            with open(f"{self.run_label}_Table_PsiP_Contamination_{target_label}.tex", "w") as f:
    ...
    def generate_overlay_plot(self, use_true_pt=False):
        ...
        prelim = ROOT.TLatex()
        prelim.SetNDC(True); prelim.SetTextColor(ROOT.kBlue); prelim.SetTextAlign(33)
        prelim.SetTextSize(0.05); prelim.DrawLatex(0.82, 0.60, "Preliminary")
        prelim.SetTextSize(0.0272); prelim.DrawLatex(0.82, 0.54, "Run Period 2014-2015")
        ...
        canvas.Update(); canvas.SaveAs(f"{self.run_label}_cross_section_overlay_Targets_vs_pT{suffix}.pdf")
    ...
    def generate_ratio_plot(self, use_true_pt=False):
        ...
        if use_true_pt:
            csv_filename = f"{self.run_label}_pd_2pp_errors.csv"
        ...
        if g_ratio_stat.GetN() > 0:
            ...
            latex_fit.SetTextFont(42); latex_fit.SetTextSize(0.04); latex_fit.SetTextColor(ROOT.kRed)
            ...
            prelim.SetNDC(True); prelim.SetTextColor(ROOT.kBlue); prelim.SetTextAlign(33)
            prelim.SetTextSize(0.05); prelim.DrawLatex(0.82, 0.60, "Preliminary")
            prelim.SetTextSize(0.0252); prelim.DrawLatex(0.82, 0.54, "Run Period 2014-2015")

            c_ratio.SaveAs(f"{self.run_label}_CrossSection_Ratio_pd_2pp_vs_pT{suffix}.pdf")
    ...
    def generate_combined_ratio_overlay_plot(self, use_true_pt=False):
        ...
        pad1.SetLeftMargin(0.16); pad1.SetRightMargin(0.05); pad1.SetTopMargin(0.08)
        ...
        g_sys_lh2_clone.SetLineColor(ROOT.kRed); g_sys_lh2_clone.SetFillColorAlpha(ROOT.kPink - 9, 0.5)
        g_xsec_lh2_clone = g_xsec_lh2.Clone()
        g_xsec_lh2_clone.SetLineColor(ROOT.kRed); g_xsec_lh2_clone.SetMarkerColor(ROOT.kRed)

        # LD2 Styling (Blue with light transparent blue band)
        g_sys_ld2_clone = g_sys_ld2.Clone()
        g_sys_ld2_clone.SetLineColor(ROOT.kBlue); g_sys_ld2_clone.SetFillColorAlpha(ROOT.kAzure + 1, 0.5) 
        g_xsec_ld2_clone = g_xsec_ld2.Clone()
        g_xsec_ld2_clone.SetLineColor(ROOT.kBlue); g_xsec_ld2_clone.SetMarkerColor(ROOT.kBlue)

        mg_top.Add(g_sys_lh2_clone, "2"); mg_top.Add(g_sys_ld2_clone, "2")
        mg_top.Add(g_xsec_lh2_clone, "P"); mg_top.Add(g_xsec_ld2_clone, "P")

        mg_top.Draw("A"); mg_top.SetTitle(""); mg_top.GetXaxis().SetLimits(0.0, 2.0)

        mg_top.GetYaxis().SetTitle("d#sigma/dp_{T} [nb/GeV]"); mg_top.GetYaxis().CenterTitle()
        mg_top.GetYaxis().SetTitleFont(43); mg_top.GetYaxis().SetTitleSize(22)
        mg_top.GetYaxis().SetTitleOffset(2.0); mg_top.GetYaxis().SetLabelFont(43); mg_top.GetYaxis().SetLabelSize(20)

        mg_top.GetXaxis().SetTitle("p_{T} [GeV]"); mg_top.GetXaxis().CenterTitle()
        mg_top.GetXaxis().SetTitleFont(43); mg_top.GetXaxis().SetTitleSize(22)
        mg_top.GetXaxis().SetTitleOffset(1.2); mg_top.GetXaxis().SetLabelFont(43); mg_top.GetXaxis().SetLabelSize(20)
        ...
        mg_top.SetMinimum(0.0); mg_top.SetMaximum(y_max * 1.4)
        ...
        pad2.SetLeftMargin(0.16); pad2.SetRightMargin(0.05)
        pad2.SetTickx(1); pad2.SetTicky(1); pad2.Draw()
        pad2.cd()
        ...
        mg_bottom.Draw("A"); mg_bottom.SetTitle(""); mg_bottom.GetXaxis().SetLimits(0.0, 2.0)
        ...
        mg_bottom.GetXaxis().SetTitle("p_{T} [GeV]"); mg_bottom.GetXaxis().CenterTitle()
        ...
        canvas.SaveAs(f"{self.run_label}_Combined_XSec_Ratio_vs_pT{suffix}.pdf")
    ...
    def generate_latex_appendix(self):
        ...
        latex_filename = f"{self.run_label}_Appendix_MassCentroids.tex"
    ...
    # =========================================================================
    # NEW FEATURE: Plotting All Roadsets against the Final Combined Average
    # =========================================================================
    @staticmethod
    def plot_roadset_overlays(roadsets, combined_label="Combined"):
        """
        Reads generated ROOT files and plots the single differential cross-sections
        of all individual roadsets overlaying the combined weighted average.
        """
        ROOT.gROOT.SetBatch(True)
        # Distinct, easy-to-differentiate colors for the 5 individual datasets
        ...
        for target in ["LH2", "LD2"]:
            for use_true_pt in [False, True]:
                ...
                c = ROOT.TCanvas(f"c_all_{target}{suffix}", f"All Roadsets {target}", 1000, 800)
                ...
                mg = ROOT.TMultiGraph()
                mg.SetTitle(f"Drell-Yan Absolute Cross-Section ({target});p_{{T}} [GeV];d#sigma/dp_{{T}} [nb/GeV/Nucleus]")
                ...
                # Track open files so graphical objects do not become null
                files = []
                max_y = 0.0

                # 1. Overlay Individual Datasets
                for idx, rs in enumerate(roadsets):
                    f = ROOT.TFile.Open(f"XSec_{rs}_Objects.root")
                    if not f or f.IsZombie(): 
                        continue
                    files.append(f)
                    
                    g_xsec = f.Get(f"CrossSections_{target}/g_xsec_{target}{suffix}")
                    g_sys = f.Get(f"CrossSections_{target}/g_sys_{target}{suffix}")
                    
                    if g_xsec and g_sys:
                        # Find maximum limit for dynamic Y scaling
                        for i in range(g_xsec.GetN()):
                            y = g_xsec.GetY()[i]
                            ey = g_sys.GetErrorY(i)
                            if y + ey > max_y: 
                        ...
                        g_sys_c = g_sys.Clone(f"sys_{rs}_{target}")
                        g_sys_c.SetLineColor(color)
                        g_sys_c.SetFillColorAlpha(color, 0.2)
                        g_sys_c.SetMarkerSize(0)
                        mg.Add(g_sys_c, "2")

                        g_xsec_c = g_xsec.Clone(f"xsec_{rs}_{target}")
                        g_xsec_c.SetLineColor(color)
                        g_xsec_c.SetMarkerColor(color)
                        g_xsec_c.SetMarkerStyle(24 + idx) # Different open markers for contrast
                        mg.Add(g_xsec_c, "P")
                        
                        leg.AddEntry(g_xsec_c, f"{rs} Data", "pl")

                # 2. Add Combined (Weighted Average) Dataset heavily accented
                f_comb = ROOT.TFile.Open(f"XSec_{combined_label}_Objects.root")
                if f_comb and not f_comb.IsZombie():
                    files.append(f_comb)
                    g_xsec_comb = f_comb.Get(f"CrossSections_{target}/g_xsec_{target}{suffix}")
                    g_sys_comb = f_comb.Get(f"CrossSections_{target}/g_sys_{target}{suffix}")
                    
                    if g_xsec_comb and g_sys_comb:
                        g_sys_cc = g_sys_comb.Clone(f"sys_comb_{target}")
                        g_sys_cc.SetLineColor(ROOT.kBlack)
                        g_sys_cc.SetFillColorAlpha(ROOT.kBlack, 0.3)
                        g_sys_cc.SetMarkerSize(0)
                        mg.Add(g_sys_cc, "2")

                        g_xsec_cc = g_xsec_comb.Clone(f"xsec_comb_{target}")
                        g_xsec_cc.SetLineColor(ROOT.kBlack)
                        g_xsec_cc.SetMarkerColor(ROOT.kBlack)
                        g_xsec_cc.SetMarkerStyle(20) # Solid black circle
                        g_xsec_cc.SetMarkerSize(1.5)
                        mg.Add(g_xsec_cc, "P")
                        
                        leg.AddEntry(g_xsec_cc, "Weighted Avg (Combined)", "pl")
                ...
                if max_y > 0: 
                    mg.SetMaximum(max_y * 1.5)
                mg.SetMinimum(0.0)
                
                leg.Draw()

                # Add Physics standard preliminary & run period labels
                prelim = ROOT.TLatex()
                prelim.SetNDC(True)
                prelim.SetTextColor(ROOT.kBlue)
                prelim.SetTextAlign(33)
                ...
                c.SaveAs(f"AllRoadsets_Overlay_{target}{suffix}.pdf")
                c.Close()
