...
class DYCrossSectionAnalyzer:
    ...
    def optimize_unfolding_iterations(self, h_raw_1d, resp_matrix, target_label, xf_bin_idx, unfold_type, max_iters=15):
        if not resp_matrix or resp_matrix.Htruth().Integral() <= 0:
            return config.UNFOLDING_ITERATIONS

        self.console.print(f"[dim]Running Iteration Optimization for {target_label} {unfold_type} xF bin {xf_bin_idx}...[/dim]")
        
        iterations = array('d')
        chi2_changes = array('d')
        avg_stat_errors = array('d')
        
        prev_hist = None
        
        for i in range(1, max_iters + 1):
            unfold = ROOT.RooUnfoldBayes(resp_matrix, h_raw_1d, i)
            h_unf = unfold.Hunfold()
            
            sum_rel_err = 0.0
            valid_bins = 0
            for b in range(1, h_unf.GetNbinsX() + 1):
                val = h_unf.GetBinContent(b)
                err = h_unf.GetBinError(b)
                if val > 0:
                    sum_rel_err += (err / val)
                    valid_bins += 1
            
            mean_stat_err = (sum_rel_err / valid_bins) * 100.0 if valid_bins > 0 else 0.0
            
            chi2_diff = 0.0
            if prev_hist is not None:
                for b in range(1, h_unf.GetNbinsX() + 1):
                    val_curr = h_unf.GetBinContent(b)
                    val_prev = prev_hist.GetBinContent(b)
                    err_curr = h_unf.GetBinError(b)
                    
                    if err_curr > 0:
                        chi2_diff += ((val_curr - val_prev) ** 2) / (err_curr ** 2)
            else:
                chi2_diff = 100.0 
            
            iterations.append(i)
            chi2_changes.append(chi2_diff)
            avg_stat_errors.append(mean_stat_err)
            
            prev_hist = h_unf.Clone(f"tmp_prev_{unfold_type}_{i}")
            prev_hist.SetDirectory(0)

        c_opt = ROOT.TCanvas(f"c_opt_{target_label}_{unfold_type}_{xf_bin_idx}", "Unfolding Optimization", 900, 700)
        c_opt.SetLeftMargin(0.12); c_opt.SetRightMargin(0.12); c_opt.SetTickx(1)
        
        g_chi2 = ROOT.TGraph(max_iters, iterations, chi2_changes)
        g_chi2.SetLineColor(ROOT.kBlue); g_chi2.SetLineWidth(3); g_chi2.SetMarkerStyle(20); g_chi2.SetMarkerColor(ROOT.kBlue)
        g_chi2.SetTitle(f"Unfold Optimization {target_label} {unfold_type} xF Bin {xf_bin_idx};Iterations;#Delta#chi^{2} to Prev Iteration")
        
        g_chi2.GetXaxis().SetLimits(1, max_iters)
        g_chi2.GetXaxis().CenterTitle(True)
        g_chi2.GetYaxis().CenterTitle(True)
        g_chi2.Draw("ALP")
        c_opt.Update()
        
        right_edge = g_chi2.GetXaxis().GetXmax()
        y_min_frame = g_chi2.GetYaxis().GetXmin()
        y_max_frame = g_chi2.GetYaxis().GetXmax()
        
        max_err = max(avg_stat_errors) if len(avg_stat_errors) > 0 else 1.0
        min_err = min(avg_stat_errors) if len(avg_stat_errors) > 0 else 0.0
        
        right_axis = ROOT.TGaxis(right_edge, y_min_frame, right_edge, y_max_frame, min_err*0.9, max_err*1.1, 510, "+L")
        right_axis.SetLineColor(ROOT.kRed); right_axis.SetLabelColor(ROOT.kRed)
        right_axis.SetTitleColor(ROOT.kRed); right_axis.SetTitle("Mean Relative Stat. Error (%)")
        right_axis.CenterTitle(True)
        right_axis.Draw()
        
        scale_factor = (y_max_frame) / (max_err * 1.1) if max_err > 0 else 1.0
        scaled_stat_errors = array('d', [e * scale_factor for e in avg_stat_errors])
        
        g_err = ROOT.TGraph(max_iters, iterations, scaled_stat_errors)
        g_err.SetLineColor(ROOT.kRed); g_err.SetLineWidth(3); g_err.SetLineStyle(7)
        g_err.SetMarkerStyle(21); g_err.SetMarkerColor(ROOT.kRed)
        g_err.Draw("LP SAME")
        
        right_margin = c_opt.GetRightMargin()
        top_margin = c_opt.GetTopMargin()
        leg_x2 = 1.0 - right_margin - 0.02
        leg_x1 = leg_x2 - 0.40
        leg_y2 = 1.0 - top_margin - 0.02
        leg_y1 = leg_y2 - 0.15

        leg = ROOT.TLegend(leg_x1, leg_y1, leg_x2, leg_y2)
        leg.SetBorderSize(0)
        leg.AddEntry(g_chi2, "#Delta#chi^{2} Convergence", "lp")
        leg.AddEntry(g_err, "Mean Stat Error Blowup", "lp")
        
        # Override dynamic selection: Explicitly lock iterations to 3
        #optimal_iter = 3
        # Explicitly lock iterations to whatever is set in config.py
        optimal_iter = config.UNFOLDING_ITERATIONS
                
        opt_line = ROOT.TLine(optimal_iter, y_min_frame, optimal_iter, y_max_frame)
        opt_line.SetLineColor(ROOT.kGreen+2); opt_line.SetLineStyle(2); opt_line.SetLineWidth(2)
        opt_line.Draw("SAME")
        leg.AddEntry(opt_line, f"Fixed Iter: {optimal_iter}", "l")
        leg.Draw()
        
        c_opt.SaveAs(f"Unfolding_Optimization_{target_label}_{unfold_type}_xF_{xf_bin_idx}.pdf")
        self.out_file.cd()
        c_opt.Write()
...
""".replace("TARGET_LABEL", target_label)
        ...
        def draw_and_save_canvas(plot_type, g_raw_xsec, g_raw_sys, g_unf_m_xsec, g_unf_m_sys, g_unf_c_xsec, g_unf_c_sys, xf_min, xf_max, xf_bin_index, theory_idx, y_min_data, y_max_data, ct18_file, nnpdf_file):
            ...
            # Pad 1 for Main Cross Section Plot
            ...
            def get_val_and_tot_err_by_x(g_stat, g_sys, x_target, tol=0.01):
                for k in range(g_stat.GetN()):
                    if abs(g_stat.GetX()[k] - x_target) < tol:
                        val = g_stat.GetY()[k]
                        stat_e = g_stat.GetErrorY(k)
                        sys_e = g_sys.GetErrorY(k)
                        tot_err = math.sqrt(stat_e**2 + sys_e**2)
                        return val, tot_err
                return None, None
            ...
            for k in range(g_raw_xsec.GetN()):
                ...
                # We skip points where Raw is zero, because we cannot divide by zero in the ratio
                if vy_raw <= 0: continue
                
                # Fetch Unfolded Messy Yield and Calculate Total Error
                vy_m, tot_err_m = get_val_and_tot_err_by_x(g_unf_m_xsec, g_unf_m_sys, vx)
                if vy_m is not None and vy_m > 0:
                    r = vy_m / vy_raw
                    # Error calculated treating Raw as a fixed zero-error baseline
                    e = tot_err_m / vy_raw 
                    g_rat_m.SetPoint(pt_m, vx, r)
                    g_rat_m.SetPointError(pt_m, 0.0, e) # Only plot Y error
                ...
                # Fetch Unfolded Clean Yield and Calculate Total Error
                vy_c, tot_err_c = get_val_and_tot_err_by_x(g_unf_c_xsec, g_unf_c_sys, vx)
                if vy_c is not None and vy_c > 0:
                    r = vy_c / vy_raw
                    # Error calculated treating Raw as a fixed zero-error baseline
                    e = tot_err_c / vy_raw
                    g_rat_c.SetPoint(pt_c, vx, r)
                    g_rat_c.SetPointError(pt_c, 0.0, e) # Only plot Y error
                    if not math.isinf(r) and not math.isnan(r) and not math.isinf(e) and not math.isnan(e):
                        rat_max = max(rat_max, r + e)
                        rat_min = min(rat_min, r - e)
                        valid_ratio_points += 1
            ...
            # Intelligent Auto-Scaling Application
