...
def process_all_datasets(file_lh2, file_ld2, file_fl, output_file):
    ...
    for i_x in range(len(xf_bins_np) - 1):
        ...
        # Track maximum Y needed for the canvases so labels don't get cut off
        max_label_y_lh2 = 0.5
        max_label_y_ld2 = 0.5
        ...
        for i_m in range(len(mass_bins_np) - 1):
            ...
            # Strict zeroing to avoid giant non-physical yields when efficiency drops out
            if eps_lh2 > 0:
            ...
            jitter_x = [-0.05, 0.05] # Horizontal zig-zag
            ...
            valid_lh2.sort(key=lambda x: x[2]) # Sort ascending by N
            
            last_y = 1e-9
            for j, (lbl, cM, N) in enumerate(valid_lh2):
                desired_y = N * 1.5
                actual_y = max(desired_y, last_y * 2.5) # Force log-space clearance
            ...
            valid_ld2.sort(key=lambda x: x[2]) # Sort ascending by N
        ...
        if overall_max_lh2 <= 0: overall_max_lh2 = 1.0 # Safety
        h1_lh2_tot.SetMaximum(overall_max_lh2 * 10.0)  # Provide top margin for labels
        h1_lh2_tot.SetMinimum(0.5) 
        
        # User requested exact title formatting: "Mass Distributions (<target type>) 0.00 <= x_{F} < 0.05"
        ...
        if overall_max_ld2 <= 0: overall_max_ld2 = 1.0 # Safety
