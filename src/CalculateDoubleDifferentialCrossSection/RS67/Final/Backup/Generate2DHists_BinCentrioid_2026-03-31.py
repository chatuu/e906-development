...
def get_correlated_mean_and_error(eff_arr, err_arr, loc_arr):
    """Calculates correlated uncertainty using the interpolation bin covariance matrix."""
    N = len(eff_arr)
    if N == 0: 
        return 0.0, 0.0
...
def extract_target_stats(data_tot, data_mix, loc_tot, loc_mix, m_low, m_high, x_low, x_high):
    """Extract yields, efficiency, and array values for a given bin with correlated errors."""
    ...
    # Incorporate the specific locs using the masks to calculate true correlated error
    means = {
    ...
    # Calculate final efficiency by multiplying the bin averages and propagating uncertainty
...
def process_all_datasets(file_lh2, file_ld2, file_fl, output_file):
    ...
    def get_loc_data(data_dict, is_mix=False):
        """Helper to compute loc_data for correlated error mapping"""
    ...
    def save_2d_pdfs(hists_dict, label):
        for name, hist in hists_dict.items():
            ...
            # --- MODIFICATION ---
            # Enforce minimum to 0 if there are negative values (e.g., negative efficiencies)
            # This ensures that empty 0.0 bins remain white in the COLZ palette.
...
def generate_subtracted_plot(hists_target, hists_flask, flask_norm, target_label, output_file):
    ...
    if has_data:
        # --- MODIFICATION ---
        # Clamp the minimum to 0 if local_min is negative to preserve white 0.0 bins
