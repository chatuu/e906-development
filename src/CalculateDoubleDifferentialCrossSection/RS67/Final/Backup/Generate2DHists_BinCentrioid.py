...
# ==========================================
# 1. Define Physics Constants & Helpers
# ==========================================
...
def get_or_create_dir(base_dir, name):
    """Helper function to create and return a TDirectory in the ROOT file."""
...
def extract_target_stats(data_tot, data_mix, loc_tot, loc_mix, m_low, m_high, x_low, x_high):
    ...
    means = {
        "r_tot": get_correlated_mean_and_error(eff_reco_tot, err_reco_tot, loc_tot[mask_tot]),
        "h_tot": get_weighted_mean_and_error(eff_hodo_tot, err_hodo_tot),
        "r_mix": get_correlated_mean_and_error(eff_reco_mix, err_reco_mix, loc_mix[mask_mix]),
...
def process_all_datasets(file_lh2, file_ld2, file_fl, output_file):
    ...
    try:
        ...
        print(f"Error loading NPZ file for Covariance generation at '{INPUT_NPZ_FILE}': {e}")
        sys.exit(1)
    ...
    def get_loc_data(data_dict, is_mix=False):
        ...
        return np.digitize(d1_vals, x_curve) - 1
    ...
    dir_kin = get_or_create_dir(output_file, "Kinematics")

    dir_lh2 = get_or_create_dir(dir_kin, "LH2")
    dir_lh2.cd()
    hists_lh2 = create_histograms("LH2")

    dir_ld2 = get_or_create_dir(dir_kin, "LD2")
    dir_ld2.cd()
    hists_ld2 = create_histograms("LD2")

    dir_fl = get_or_create_dir(dir_kin, "Flask")
    dir_fl.cd()
    ...
    dir_mass1d = get_or_create_dir(output_file, "Mass_Distributions_1D")
    dir_csv = get_or_create_dir(output_file, "CSV_Tables")
    ...
    for i_x in range(len(xf_bins_np) - 1):
        ...
        for i_m in range(len(mass_bins_np) - 1):
            ...
            N_l2_t, N_l2_m, M_l2_t, M_l2_m, eps_ld2, err_eps_ld2, diff_l2, mean_l2 = extract_target_stats(data_ld2_tot, data_ld2_mix, loc_ld2_tot, loc_ld2_mix, m_low, m_high, x_low, x_high)
            N_lh_t, N_lh_m, M_lh_t, M_lh_m, eps_lh2, err_eps_lh2, diff_lh, mean_lh = extract_target_stats(data_lh2_tot, data_lh2_mix, loc_lh2_tot, loc_lh2_mix, m_low, m_high, x_low, x_high)
            N_fl_t, N_fl_m, M_fl_t, M_fl_m, eps_fl, err_eps_fl, diff_fl, mean_fl = extract_target_stats(data_fl_tot, data_fl_mix, loc_fl_tot, loc_fl_mix, m_low, m_high, x_low, x_high)
        ...
        # Write CSV files into ROOT TDirectory using TMacro
        dir_csv.cd()
        ...
        print(f"  Saved LH2 & LD2 1D Mass Dists, generated CSVs, and embedded them inside ROOT file for xF: [{x_low:.2f}, {x_high:.2f})")
...
def generate_subtracted_plot(hists_target, hists_flask, flask_norm, target_label, output_file):
    ...
    dir_sub = get_or_create_dir(output_file, f"Subtracted_Plots_{target_label}")
    dir_sub.cd()
...
def generate_pd_subtracted_plot(hists_ld2, hists_lh2, hists_flask, output_file):
    ...
    dir_sub = get_or_create_dir(output_file, "Subtracted_Plots_LD2_pd")
    dir_sub.cd()
...
def calculate_and_plot_cross_section(h_sub_dict, target_label, global_constant, output_file):
    ...
    dir_xsec = get_or_create_dir(output_file, f"CrossSections_{target_label}")
    dir_xsec.cd()
...
""" % (target_label, target_label)
    ...
    def draw_and_save_canvas(plot_type, g_xsec, g_sys, h1_xsec, h1_sys, xf_min, xf_max, xf_bin_index, y_min_data, y_max_data):
        ...
        c_xsec.Close()
        print(f"  Saved {plot_type} plot: {plot_name} and linked objects inside TDirectory.")
    ...
    for i_x in range(n_xf_bins):
        ...
        # Adding TH1Ds for Centroid
        h1_xsec_cent = ROOT.TH1D(f"h1_xsec_cent_{target_label}_xF_{xf_bin_index}", f"Data-Driven Cross Section {target_label} xF={xf_bin_index};Mass [GeV];M^3 d^2#sigma/dMdx_F", n_mass_bins, mass_bins_np)
        h1_sys_cent = ROOT.TH1D(f"h1_sys_cent_{target_label}_xF_{xf_bin_index}", f"Systematic Error Band (Centroid) {target_label} xF={xf_bin_index};Mass [GeV];M^3 d^2#sigma/dMdx_F", n_mass_bins, mass_bins_np)
        ...
        # Adding TH1Ds for GeoCenter
        h1_xsec_geo = ROOT.TH1D(f"h1_xsec_geo_{target_label}_xF_{xf_bin_index}", f"GeoCenter Cross Section {target_label} xF={xf_bin_index};Mass [GeV];M^3 d^2#sigma/dMdx_F", n_mass_bins, mass_bins_np)
        h1_sys_geo = ROOT.TH1D(f"h1_sys_geo_{target_label}_xF_{xf_bin_index}", f"Systematic Error Band (GeoCenter) {target_label} xF={xf_bin_index};Mass [GeV];M^3 d^2#sigma/dMdx_F", n_mass_bins, mass_bins_np)
        ...
        for i_m in range(n_mass_bins):
            ...
            # Populate Data-Driven Centroid Graphs & TH1Ds
            ...
            # Populate Geometric Center Graphs & TH1Ds
...
def generate_overlay_plot(out_file, target_label, plot_type):
    ...
    n_xf_bins = len(xf_bins_np) - 1
    dir_xsec = out_file.Get(f"CrossSections_{target_label}")
    if not dir_xsec:
        print(f"Error: Could not find CrossSections_{target_label} directory for overlay calculation.")
        return

    # Iterate and extract graphs directly from the proper output directory
    ...
    dir_overlay = get_or_create_dir(out_file, "Overlays")
    dir_overlay.cd()
    canvas.Write(f"canvas_overlay_{target_label}_{plot_type}")
    print(f"  Overlay plot saved to {out_pdf} and written to Overlays TDirectory.")
...
def main():
    output_filename = "All_XSec_Objects.root"
    ...
    hists_lh2, hists_ld2, hists_flask = process_all_datasets(
        "../../../HodoEfficiency/RS67/AllTargets/merged_RS67_3089_LH2_recoeff_hodoeff.root",
        "../../../HodoEfficiency/RS67/AllTargets/merged_RS67_3089_LD2_recoeff_hodoeff.root",
        "../../../HodoEfficiency/RS67/AllTargets/merged_RS67_3089_Flask_recoeff_hodoeff.root",
        out_file
    )
