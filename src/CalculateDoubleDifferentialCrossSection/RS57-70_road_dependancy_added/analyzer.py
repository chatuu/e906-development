...
class DYCrossSectionAnalyzer:
    ...
    def load_roadset_sys_csv(self, csv_path):
        """
        Robustly parses CSV by skipping title lines and fuzzy-matching exact bin indices.
        """
        sys_data = {}
        if not os.path.exists(csv_path):
            print(f"Warning: Road-dependency CSV not found at {csv_path}")
            return sys_data
        
        with open(csv_path, 'r') as f:
            lines = f.readlines()
            
        # Find the actual header row to bypass title strings
        header_idx = -1
        for i, line in enumerate(lines):
            if "xF" in line and "Mass" in line:
                header_idx = i
                break
                
        if header_idx == -1:
            print(f"Warning: Could not find valid headers in {csv_path}")
            return sys_data
            
        reader = csv.DictReader(lines[header_idx:])
        for row in reader:
            xf_idx, mass_idx = None, None
            mean_val, std_val, perc_val = 0.0, 0.0, 0.0
            ...
            for k, v in row.items():
                ...
                if "xfbinindex" in k_clean:
                    try: xf_idx = int(float(v_clean))
                    except: pass
                elif "massbinindex" in k_clean:
                    try: mass_idx = int(float(v_clean))
                    except: pass
                elif "meancrosssection" in k_clean:
                    try: mean_val = float(v_clean)
                    except: pass
                elif "standarddeviation" in k_clean or "stddev" in k_clean:
                    try: std_val = float(v_clean)
                    except: pass
                elif "relativesys" in k_clean or "percentage" in k_clean:
                    try: perc_val = float(v_clean)
                    except: pass

            if xf_idx is not None and mass_idx is not None:
                sys_data[(xf_idx, mass_idx)] = {
                    'mean': mean_val,
                    'std': std_val,
                    'rel_perc': perc_val
                }
    ...
    def calculate_and_plot_cross_section(self, h_sub_dict, target_label, global_constant):
        ...
        if target_label == "LD2":
            ...
            csv_path = config.CSV_ROADSET_SYS_LD2
        else:
            theory_ct18_path = "CT18_xFnew_p_1sigma.root"
            theory_nnpdf_path = "NNPDF40_xFnew_p.root"
            csv_path = config.CSV_ROADSET_SYS_LH2
            
        csv_data = self.load_roadset_sys_csv(csv_path)
...
""" % (target_label, target_label)
        ...
        for i_x in range(n_xf_bins):
            ...
            for i_m in range(n_mass_bins):
                ...
                key = (xf_bin_index, i_m)
                matched_csv = csv_data.get(key, None)

                # Prioritize CSV data unconditionally if it has a mean > 0
                if matched_csv and matched_csv['mean'] > 0.0:
                    scaled_xsec = matched_csv['mean']
                    
                    fractional_sys_road = matched_csv['rel_perc'] / 100.0
                    scaled_sys_road = fractional_sys_road * scaled_xsec
                    
                    frac_stat = (Y_sub_stat_err / Y_sub) if Y_sub > 0 else 0.0
                    frac_yield = (Y_sub_sys_err / Y_sub) if Y_sub > 0 else 0.0
                    frac_acc = (acceptance_err / acceptance) if acceptance > 0 else 0.0
                    
                    scaled_stat_err = frac_stat * scaled_xsec
                    ...
                    raw_xsec = numerator / denominator
                    
                    raw_stat_unc = (Y_sub_stat_err / Y_sub) * raw_xsec
                    ...
                    scaled_stat_err = raw_stat_unc * scaling_factor
                    scaled_sys_err = raw_sys_err * scaling_factor
                    scaled_sys_psip = raw_sys_psip * scaling_factor
                
                plot_data.append({
                    'mass_geom': geometric_center,
                    'mass_act': actual_mass_center,
                    'mass_width': mass_width,
                    'xsec': scaled_xsec,
                    'stat_err': scaled_stat_err,
                    'sys_err': scaled_sys_err,
                    'psip_ratio': psip_ratio,
                    'psip_ratio_err': psip_ratio_err,
                    'scaled_sys_psip': scaled_sys_psip
                })
            if not plot_data: continue

            plot_y_min = sys.float_info.max
            plot_y_max = -sys.float_info.max
            
            for p in plot_data:
                if p['xsec'] <= 0:
                    continue
                    
                actual_bottom = p['xsec'] - p['sys_err']
                if actual_bottom <= 0:
                    actual_bottom = p['xsec'] * 0.001 
                
                if actual_bottom < plot_y_min: plot_y_min = actual_bottom
                if p['xsec'] + p['sys_err'] > plot_y_max: plot_y_max = p['xsec'] + p['sys_err']
            ...
            g_xsec_cent = ROOT.TGraphAsymmErrors()
            g_xsec_cent.SetName(f"g_xsec_cent_{target_label}_xF_{xf_bin_index}")
            g_sys_cent = ROOT.TGraphAsymmErrors()
            ...
            g_xsec_geo = ROOT.TGraphAsymmErrors()
            g_xsec_geo.SetName(f"g_xsec_geo_{target_label}_xF_{xf_bin_index}")
            g_sys_geo = ROOT.TGraphAsymmErrors()
            ...
            point_idx = 0
            for p in plot_data:
                if p['xsec'] <= 0: continue
                    
                if p['psip_ratio'] > 0.0:
                    mass_min = p['mass_geom'] - p['mass_width']/2.0
                    mass_max = p['mass_geom'] + p['mass_width']/2.0
                    s_xf = f"[{xf_min:.2f}, {xf_max:.2f})"
                    s_mass = f"[{mass_min:.2f}, {mass_max:.2f})"
                    s_ratio = f"{p['psip_ratio']:.4f} $\\pm$ {p['psip_ratio_err']:.4f}"
                    s_sigma_psip_col = f"{p['xsec']:.4f} $\\pm$ {p['stat_err']:.4f} $\\pm$ {p['sys_err']:.4f}"
                    latex_psip_table_content += f"{s_xf} & {s_mass} & {s_ratio} & {s_sigma_psip_col} & {p['scaled_sys_psip']:.4f} \\\\ \n\\hline\n"
                
                sys_ey_high = p['sys_err']
                sys_ey_low = p['sys_err']
                if p['xsec'] - sys_ey_low <= 0:
                    sys_ey_low = p['xsec'] * 0.999 

                stat_ey_high = p['stat_err']
                stat_ey_low = p['stat_err']
                if p['xsec'] - stat_ey_low <= 0:
                    stat_ey_low = p['xsec'] * 0.999 

                # Calculate exact distances to true bin edges
                mass_min = p['mass_geom'] - (p['mass_width'] / 2.0)
                mass_max = p['mass_geom'] + (p['mass_width'] / 2.0)
                
                ex_low_cent = p['mass_act'] - mass_min
                ex_high_cent = mass_max - p['mass_act']
                
                ex_low_geo = p['mass_geom'] - mass_min
                ex_high_geo = mass_max - p['mass_geom']

                # --- FILL CENTROID GRAPHS ---
                g_xsec_cent.SetPoint(point_idx, p['mass_act'], p['xsec'])
                g_xsec_cent.SetPointError(point_idx, ex_low_cent, ex_high_cent, stat_ey_low, stat_ey_high)
                
                g_sys_cent.SetPoint(point_idx, p['mass_act'], p['xsec']) 
                g_sys_cent.SetPointError(point_idx, ex_low_cent, ex_high_cent, sys_ey_low, sys_ey_high)
                
                h1_xsec_cent.SetBinContent(point_idx+1, p['xsec'])
                h1_xsec_cent.SetBinError(point_idx+1, p['stat_err'])
                h1_sys_cent.SetBinContent(point_idx+1, p['xsec'])
                h1_sys_cent.SetBinError(point_idx+1, p['sys_err'])

                # --- FILL GEOCENTER GRAPHS ---
                g_xsec_geo.SetPoint(point_idx, p['mass_geom'], p['xsec'])
                g_xsec_geo.SetPointError(point_idx, ex_low_geo, ex_high_geo, stat_ey_low, stat_ey_high)
                
                g_sys_geo.SetPoint(point_idx, p['mass_geom'], p['xsec'])
                g_sys_geo.SetPointError(point_idx, ex_low_geo, ex_high_geo, sys_ey_low, sys_ey_high)

                h1_xsec_geo.SetBinContent(point_idx+1, p['xsec'])
                h1_xsec_geo.SetBinError(point_idx+1, p['stat_err'])
                h1_sys_geo.SetBinContent(point_idx+1, p['xsec'])
                h1_sys_geo.SetBinError(point_idx+1, p['sys_err'])

                point_idx += 1

            if g_xsec_cent.GetN() > 0:
                draw_and_save_canvas("Centroid", g_xsec_cent, g_sys_cent, h1_xsec_cent, h1_sys_cent, xf_min, xf_max, xf_bin_index, pad_y_min, pad_y_max)
                draw_and_save_canvas("GeoCenter", g_xsec_geo, g_sys_geo, h1_xsec_geo, h1_sys_geo, xf_min, xf_max, xf_bin_index, pad_y_min, pad_y_max)
    ...
    def generate_overlay_plot(self, target_label, plot_type):
        def scale_tgrapherrors(g, scale):
            ...
            if g.InheritsFrom("TGraphAsymmErrors"):
                for i in range(g.GetN()):
                    x = x_buf[i]; y = y_buf[i]
                    ex_l = g.GetErrorXlow(i); ex_h = g.GetErrorXhigh(i)
                    ey_l = g.GetErrorYlow(i); ey_h = g.GetErrorYhigh(i)
                    g.SetPoint(i, x, y * scale)
                    g.SetPointError(i, ex_l, ex_h, ey_l * scale, ey_h * scale)
            else:
                for i in range(g.GetN()):
                    x = x_buf[i]; y = y_buf[i]
        ...
        h_frame = canvas.DrawFrame(overlay_x_min, overlay_y_min, overlay_x_max, overlay_y_max)
        ...
        for i in range(n_xf_bins):
            ...
            # Systematics explicitly removed from the multi-overlay plot to keep it clean
            g_xsec_clone.Draw("PE SAME")
        ...
        if target_label == "LD2" or target_label == "LH2":
            ...
            if target_label == "LH2": road_sys_txt = "4.4%"
            elif target_label == "LD2": road_sys_txt = "5.4%"
            else: road_sys_txt = "X%"
    ...
    def generate_latex_appendix(self):
        ...
        with open(latex_filename, "w") as tex_file:
            tex_file.write(r"""\section{Appendix: Determination of Mass Bin Centroids}...""")
