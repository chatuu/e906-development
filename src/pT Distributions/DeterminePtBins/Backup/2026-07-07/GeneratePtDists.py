...
def extract_kinematics(filepaths, is_mc=False, treename="Tree"):
    """Reads a list of root files, applies cuts, and returns (pT_array, weight_array)."""
    ...
    for filepath in filepaths:
        ...
        if is_mc:
            try:
                # Attempt to extract kinematics and ReWeight for MC
                events = tree.arrays(["dpx", "dpy", "ReWeight"], library="np")
                weights = events["ReWeight"][mask]
            except uproot.exceptions.KeyInFileError:
                print(f"  -> WARNING: 'ReWeight' branch missing in {filepath}. Defaulting to 1.0.")
                events = tree.arrays(["dpx", "dpy"], library="np")
                weights = np.ones(np.count_nonzero(mask), dtype=np.float64)
        else:
            # Data/Mixed always get a weight of 1.0
            events = tree.arrays(["dpx", "dpy"], library="np")
            weights = np.ones(np.count_nonzero(mask), dtype=np.float64)
    ...
    if len(all_pts) > 0:
        return np.concatenate(all_pts), np.concatenate(all_weights)
...
def make_subtracted_normalized_th1(name, title, xtitle, data_arr, w_data, mix_arr, w_mix, color, bins, xmin, xmax):
    ...
    # Subtract Mixed from Data
    h_final = h_data.Clone(name)
    h_final.Add(h_mix, -1)
    
    h_final.GetXaxis().SetTitle(xtitle)
    h_final.GetYaxis().SetTitle("Normalized Yield")
    ...
    # Normalize subtracted distribution
...
def plot_variable_canvas(target_name, var_name, xtitle, 
                         ...
                         psi_arr, w_psi, 
                         bins, xmin, xmax, ymax):
    """Generates and saves the plot for a specific variable using Data - Mixed logic and weighted MC."""
    c_name = f"c_{target_name}_{var_name}"
    c = ROOT.TCanvas(c_name, f"{target_name} {var_name} Distribution", 800, 600)
    ...
    h_jpsi = make_normalized_th1(f"h_jpsi_{c_name}", "Messy J/#psi", xtitle, jpsi_arr, w_jpsi, ROOT.kBlue, bins, xmin, xmax)
    h_psi  = make_normalized_th1(f"h_psi_{c_name}", "Messy #psi'", xtitle, psi_arr, w_psi, ROOT.kGreen+2, bins, xmin, xmax)
    ...
    h_data.Draw("E1")
    h_dy.Draw("HIST SAME")
    h_jpsi.Draw("HIST SAME")
    h_psi.Draw("HIST SAME")

    leg = ROOT.TLegend(0.65, 0.65, 0.88, 0.88)
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.AddEntry(h_data, f"Data - Mix ({target_name})", "lep")
    leg.AddEntry(h_dy, "Messy DY MC", "l")
    leg.AddEntry(h_jpsi, "Messy J/#psi MC", "l")
    leg.AddEntry(h_psi, "Messy #psi' MC", "l")
    leg.Draw()

    pdf_filename = f"{var_name}_Distribution_{target_name}.pdf"
    c.SaveAs(pdf_filename)
    print(f"Successfully saved: {pdf_filename} (Y-range: [0, {ymax}])")
    
    return c, h_data, h_dy, h_jpsi, h_psi, leg
...
mc_jpsi_lh2 = os.path.join(base_dir, "mc_jpsi_LH2_M027_S001_messy_occ_pTxFweight_v2.root")
mc_psi_lh2  = os.path.join(base_dir, "mc_psiprime_LH2_M027_S001_messy_occ_pTxFweight_v2.root")

# LD2 MC Files
mc_dy_ld2   = os.path.join(base_dir, "mc_drellyan_LD2_M027_S001_messy_occ_pTxFweight_v2.root")
mc_jpsi_ld2 = os.path.join(base_dir, "mc_jpsi_LD2_M027_S001_messy_occ_pTxFweight_v2.root")
mc_psi_ld2  = os.path.join(base_dir, "mc_psiprime_LD2_M027_S001_messy_occ_pTxFweight_v2.root")

# --- Extract Data Arrays ---
print("--- Extracting LH2 Data & Mixed Background ---")
pt_data_lh2, w_data_lh2 = extract_kinematics(lh2_files, is_mc=False, treename="result")
pt_mix_lh2, w_mix_lh2   = extract_kinematics(lh2_files, is_mc=False, treename="result_mix")

print("\n--- Extracting LD2 Data & Mixed Background ---")
pt_data_ld2, w_data_ld2 = extract_kinematics(ld2_files, is_mc=False, treename="result")
pt_mix_ld2, w_mix_ld2   = extract_kinematics(ld2_files, is_mc=False, treename="result_mix")

print("\n--- Extracting LH2 Messy MC ---")
pt_dy_lh2, w_dy_lh2     = extract_kinematics(mc_dy_lh2, is_mc=True, treename="Tree")
pt_jpsi_lh2, w_jpsi_lh2 = extract_kinematics(mc_jpsi_lh2, is_mc=True, treename="Tree")
pt_psi_lh2, w_psi_lh2   = extract_kinematics(mc_psi_lh2, is_mc=True, treename="Tree")

print("\n--- Extracting LD2 Messy MC ---")
pt_dy_ld2, w_dy_ld2     = extract_kinematics(mc_dy_ld2, is_mc=True, treename="Tree")
pt_jpsi_ld2, w_jpsi_ld2 = extract_kinematics(mc_jpsi_ld2, is_mc=True, treename="Tree")
pt_psi_ld2, w_psi_ld2   = extract_kinematics(mc_psi_ld2, is_mc=True, treename="Tree")

# --- Generate Plots ---
print("\n--- Generating pT Canvases (Data - Mixed) and saving to PDFs ---")

# Save objects in a list to prevent Python's garbage collector from destroying ROOT elements prematurely
plot_refs = []

# LH2 pT Plot
plot_refs.append(plot_variable_canvas(
    "LH2", "pT", "p_{T} [GeV/c]", 
    pt_data_lh2, w_data_lh2, 
    pt_mix_lh2, w_mix_lh2, 
    pt_dy_lh2, w_dy_lh2, 
    pt_jpsi_lh2, w_jpsi_lh2, 
    pt_psi_lh2, w_psi_lh2, 
    bins=50, xmin=0.0, xmax=2.5, ymax=0.10
))

# LD2 pT Plot
plot_refs.append(plot_variable_canvas(
    "LD2", "pT", "p_{T} [GeV/c]", 
    pt_data_ld2, w_data_ld2, 
    pt_mix_ld2, w_mix_ld2, 
    pt_dy_ld2, w_dy_ld2, 
    pt_jpsi_ld2, w_jpsi_ld2, 
    pt_psi_ld2, w_psi_ld2, 
    bins=50, xmin=0.0, xmax=2.5, ymax=0.10
))

print("\nDone! Exactly 2 PDFs (LH2 and LD2 pT distributions) with Data-Mixed backgrounds have been created.")