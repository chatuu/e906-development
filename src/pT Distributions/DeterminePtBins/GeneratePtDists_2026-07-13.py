...
def make_normalized_th1(name, title, xtitle, data_array, weight_array, color, bins, xmin, xmax):
    ...
    h.Sumw2() # Explicitly instruct ROOT to track sum of weights squared for correct errors
    h.SetLineColor(color)
    h.SetMarkerColor(color)
    h.SetMarkerStyle(1) # Simple marker so error bars look clean
    h.SetLineWidth(2)
    h.SetStats(0)
...
def make_subtracted_normalized_th1(name, title, xtitle, data_arr, w_data, mix_arr, w_mix, color, bins, xmin, xmax):
    ...
    # Subtract Mixed from Data (errors combine in quadrature implicitly because of Sumw2)
    ...
    # Normalize subtracted distribution (errors scale linearly implicitly)
...
def plot_variable_canvas(target_name, var_name, xtitle, 
    ...
    # Make canvas slightly taller to accommodate the ratio plot
    c = ROOT.TCanvas(c_name, f"{target_name} {var_name} Distribution", 800, 800)
    
    # --- Pad 1: Main Distributions (Top 70%) ---
    pad1 = ROOT.TPad(f"pad1_{c_name}", "pad1", 0.0, 0.3, 1.0, 1.0)
    pad1.SetBottomMargin(0.02)  # Join seamlessly with bottom pad
    ...
    # Format Main Axes (Hide X-axis labels for the upper pad)
    h_data.SetMinimum(0.0)
    h_data.SetMaximum(ymax)
    h_data.GetXaxis().SetLabelSize(0)
    h_data.GetXaxis().SetTitleSize(0)
    
    # Use absolute pixel sizes for Y-axis to match the lower pad later
    h_data.GetYaxis().SetTitle("Normalized Yield")
    h_data.GetYaxis().CenterTitle(True) # Added Center Title
    ...
    # Draw Histograms
    h_data.Draw("E1")
    # Draw DY Histogram outline first, then draw its error bars explicitly on top
    h_dy.Draw("HIST SAME")
    h_dy.Draw("E1 SAME")
    ...
    leg.AddEntry(h_dy, "Messy DY MC", "le") # 'le' shows line & error in legend
    leg.Draw()

    c.cd() # Return to main canvas
    
    # --- Pad 2: Ratio Plot (Bottom 30%) ---
    pad2 = ROOT.TPad(f"pad2_{c_name}", "pad2", 0.0, 0.0, 1.0, 0.3)
    pad2.SetTopMargin(0.04)    # Join seamlessly with top pad
    pad2.SetBottomMargin(0.35) # Leave space for X-axis labels
    ...
    # Calculate Ratio: (Data - Mix) / Messy DY
    # Divide automatically propagates uncorrelated errors because Sumw2 is active on both
    h_ratio = h_data.Clone(f"h_ratio_{c_name}")
    h_ratio.SetTitle("")
    h_ratio.Divide(h_dy)
    
    # Format Ratio Axes (Using absolute pixel size font=43 so it matches pad1)
    h_ratio.GetYaxis().SetTitle("Data / DY")
    h_ratio.GetYaxis().CenterTitle(True) # Added Center Title
    h_ratio.GetYaxis().SetNdivisions(505) # Clean up Y-axis ticks
    ...
    h_ratio.GetXaxis().CenterTitle(True) # Added Center Title
    ...
    # Set Ratio Y-axis Limits (adjust as necessary)
    h_ratio.SetMinimum(0.0)
    h_ratio.SetMaximum(2.5)
    
    h_ratio.SetMarkerStyle(20)
    h_ratio.SetMarkerSize(0.8)
    # 'ep' plots both the marker and the correct error bar
    h_ratio.Draw("ep")
    ...
    # Return all ROOT objects so Python doesn't garbage collect them before drawing
...
plot_refs.append(plot_variable_canvas(
    ...
    pt_dy_lh2, w_dy_lh2, 
    bins=50, xmin=0.0, xmax=2.5, ymax=0.07 # Updated to 0.07 here
))
...
print("\nDone! Exactly 2 PDFs (LH2 and LD2 pT distributions) with Data-Mixed backgrounds and Ratio plots have been created.")