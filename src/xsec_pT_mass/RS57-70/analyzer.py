...
class DYCrossSectionAnalyzer:
    ...
    def calculate_and_plot_cross_section(self, h_sub_dict, target_label, global_constant):
        ...
        if target_label == "LD2":
            theory_ct18_path = "CT18_pT_d_1sigma.root"
            theory_nnpdf_path = "NNPDF40_pT_d.root"
        else:
            theory_ct18_path = "CT18_pT_p_1sigma.root"
            theory_nnpdf_path = "NNPDF40_pT_p.root"
...
\hline
\textbf{pT bin} & \textbf{Mass bin (GeV)} & \textbf{$\psi'$ contamination} & \textbf{$\sigma \pm \delta \sigma^{stat.} \pm \delta \sigma^{syst.}$} & \textbf{$\delta\sigma_{\psi'}^{\rm syst.}$ (nb GeV$^2$)} \\
\hline
\endfirsthead
...
""" % (target_label, target_label)
        ...
        def draw_and_save_canvas(plot_type, g_xsec, g_sys, h1_xsec, h1_sys, pt_min, pt_max, pt_bin_index, y_min_data, y_max_data):
            c_xsec = ROOT.TCanvas(f"c_xsec_{target_label}_{pt_bin_index}_{plot_type}", "", 800, 600)
            c_xsec.SetLogy(); c_xsec.SetTickx(1); c_xsec.SetTicky(1)
            
            mg = ROOT.TMultiGraph()
            mg.SetTitle(f";Mass [GeV];M^{{3}} d^{{2}}\\sigma / dM dp_{{T}} [nb GeV^{{2}}/Nucleus]")
            ...
            leg.SetBorderSize(0)
            gr_name = f"gr_pTbin{pt_bin_index}"
            ...
            internal_title.DrawLatex(0.14, 0.86, f"Drell-Yan process in {target_prefix} at {pt_min:.2f} #leq p_{{T}} < {pt_max:.2f}")
            ...
            plot_name = f"CrossSection_{target_label}_pT_{pt_min:.2f}_{pt_max:.2f}_{plot_type}.pdf"
            c_xsec.SaveAs(plot_name)
            
            dir_xsec.cd()
            g_xsec.Write(f"g_xsec_{target_label}_{pt_bin_index}_{plot_type}")
            g_sys.Write(f"g_sys_{target_label}_{pt_bin_index}_{plot_type}")
            h1_xsec.Write()
            h1_sys.Write()
            c_xsec.Write(f"c_xsec_{target_label}_{pt_bin_index}_{plot_type}")
            c_xsec.Close()
        ...
        for i_pt in range(n_pt_bins):
            ...
            acc_dir = acc_file.Get(f"pT_bin{pt_bin_index}")
            if not acc_dir: 
                print(f"Warning: TDirectory 'pT_bin{pt_bin_index}' not found. Skipping bin.")
                continue

            h_acc = acc_dir.Get(f"h_ratio_{target_label}_pT_bin{pt_bin_index}")
            if not h_acc: 
                h_acc = acc_dir.Get(f"h_ratio_LH2_pT_bin{pt_bin_index}")
            if not h_acc: 
                print(f"Warning: Acceptance histogram not found for pT bin {pt_bin_index}. Skipping bin.")
                continue
            
            h_ratio_psip = f_psip.Get(f"hRatio_PsiP_DY_pT_{pt_bin_index}") if f_psip else None

            dir_xsec.cd()
            g_xsec_cent = ROOT.TGraphErrors()
            g_xsec_cent.SetName(f"g_xsec_cent_{target_label}_pT_{pt_bin_index}")
            g_sys_cent = ROOT.TGraphErrors()
            g_sys_cent.SetName(f"g_sys_cent_{target_label}_pT_{pt_bin_index}")

            h1_xsec_cent = ROOT.TH1D(f"h1_xsec_cent_{target_label}_pT_{pt_bin_index}", f"Data-Driven Cross Section {target_label} pT={pt_bin_index};Mass [GeV];M^3 d^2#sigma/dMdp_T", n_mass_bins, config.MASS_BINS)
            h1_sys_cent = ROOT.TH1D(f"h1_sys_cent_{target_label}_pT_{pt_bin_index}", f"Systematic Error Band (Centroid) {target_label} pT={pt_bin_index};Mass [GeV];M^3 d^2#sigma/dMdp_T", n_mass_bins, config.MASS_BINS)

            g_xsec_geo = ROOT.TGraphErrors()
            g_xsec_geo.SetName(f"g_xsec_geo_{target_label}_pT_{pt_bin_index}")
            g_sys_geo = ROOT.TGraphErrors()
            g_sys_geo.SetName(f"g_sys_geo_{target_label}_pT_{pt_bin_index}")
            
            h1_xsec_geo = ROOT.TH1D(f"h1_xsec_geo_{target_label}_pT_{pt_bin_index}", f"GeoCenter Cross Section {target_label} pT={pt_bin_index};Mass [GeV];M^3 d^2#sigma/dMdp_T", n_mass_bins, config.MASS_BINS)
            h1_sys_geo = ROOT.TH1D(f"h1_sys_geo_{target_label}_pT_{pt_bin_index}", f"Systematic Error Band (GeoCenter) {target_label} pT={pt_bin_index};Mass [GeV];M^3 d^2#sigma/dMdp_T", n_mass_bins, config.MASS_BINS)
            ...
            for i_m in range(n_mass_bins):
                ...
                # Include dynamic pT width in the denominator
                numerator = global_constant * Y_sub
                denominator = mass_width * pt_width * acceptance
                ...
                # Filter out cross-section data points with fractional uncertainty exceeding 0.99
                if scaled_xsec > 0 and (max_err_for_range / scaled_xsec) > 0.99:
                    continue
                ...
                if psip_ratio > 0.0:
                    ...
                    latex_psip_table_content += f"{s_pt} & {s_mass} & {s_ratio} & {s_sigma_psip_col} & {scaled_sys_psip:.4f} \\\\ \n\\hline\n"
            ...
            if g_xsec_cent.GetN() > 0:
                draw_and_save_canvas("Centroid", g_xsec_cent, g_sys_cent, h1_xsec_cent, h1_sys_cent, pt_min, pt_max, pt_bin_index, y_min_data, y_max_data)
                draw_and_save_canvas("GeoCenter", g_xsec_geo, g_sys_geo, h1_xsec_geo, h1_sys_geo, pt_min, pt_max, pt_bin_index, y_min_data, y_max_data)
    ...
    def generate_overlay_plot(self, target_label, plot_type):
        ...
        legend.SetHeader("p_{T} Bins"); legend.SetBorderSize(0); legend.SetFillStyle(0)
        ...
        h_frame.SetTitle(f"DY Absolute Cross-Section Vs Mass for p_{{T}} bins ({target_label})")
        h_frame.GetXaxis().SetTitle("Invariant Mass (GeV)"); h_frame.GetXaxis().CenterTitle()
        h_frame.GetXaxis().SetTitleOffset(1.2); h_frame.GetYaxis().SetTitle("M^{3} #frac{d^{2}#sigma}{dMdp_{T}} (nb GeV^{2}) #times 5 #times 10^{2 #times p_{T} bin index}")
        ...
        for i in range(n_pt_bins):
            ...
            low_edge, high_edge = config.PT_BINS[i], config.PT_BINS[i+1]
            y_pos = 1.0 * scale_factor
            
            latex = ROOT.TLatex(3.1, y_pos, f"{low_edge:.2f} #leq p_{{T}} < {high_edge:.2f}")
            latex.SetTextFont(43); latex.SetTextSize(20); latex.SetTextColor(color)
            latex.Draw()
            latex_labels.append(latex) 
            legend.AddEntry(g_xsec_clone, f"p_{{T}} bin {i}", "pl")
