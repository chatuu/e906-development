...
def main():
    ...
    # Configuration
    TEX_FILE = "DY_Unfolding_Update.tex"
    PLOT_TYPE = "GeoCenter" # Matches the Canvas names generated by analyzer.py
    
    # Absolute path to where the plots are generated
    IMAGE_DIR = "/root/github/e906-development/src/CalculateDoubleDifferentialCrossSection/RS57-70_with_unfolding/"
...
""")
        ...
        # --- DOUBLE DIFFERENTIAL CROSS SECTION SLIDES ---
        for i in range(len(XF_BINS) - 1):
            xf_min = XF_BINS[i]
            xf_max = XF_BINS[i+1]
            lh2_plot = f"CrossSection_LH2_xF_{xf_min:.2f}_{xf_max:.2f}_{PLOT_TYPE}.pdf"
            ld2_plot = f"CrossSection_LD2_xF_{xf_min:.2f}_{xf_max:.2f}_{PLOT_TYPE}.pdf"
            
            f.write(f"""
\\begin{{frame}}{{{xf_min:.2f} $\\leq x_F <$ {xf_max:.2f}}}
    \\begin{{columns}}
        \\begin{{column}}{{0.5\\textwidth}}
            \\centering
            \\textbf{{LH2 Target}}\\\\
            \\includegraphics[width=\\textwidth,height=0.75\\textheight,keepaspectratio]{{{lh2_plot}}}
        \\end{{column}}
        \\begin{{column}}{{0.5\\textwidth}}
            \\centering
            \\textbf{{LD2 Target}}\\\\
            \\includegraphics[width=\\textwidth,height=0.75\\textheight,keepaspectratio]{{{ld2_plot}}}
        \\end{{column}}
    \\end{{columns}}
\\end{{frame}}
""")
...
""")
        ...
        for unfold_type in ["Messy", "Clean"]:
            for i in range(len(XF_BINS) - 1):
                xf_min = XF_BINS[i]
                xf_max = XF_BINS[i+1]
                lh2_opt = f"Unfolding_Optimization_LH2_{unfold_type}_xF_{i}.pdf"
                ld2_opt = f"Unfolding_Optimization_LD2_{unfold_type}_xF_{i}.pdf"
                
                f.write(f"""
\\begin{{frame}}{{Opt ({unfold_type}): {xf_min:.2f} $\\leq x_F <$ {xf_max:.2f}}}
    \\begin{{columns}}
        \\begin{{column}}{{0.5\\textwidth}}
            \\centering
            \\textbf{{LH2 ({unfold_type})}}\\\\
            \\includegraphics[width=\\textwidth,height=0.75\\textheight,keepaspectratio]{{{lh2_opt}}}
        \\end{{column}}
        \\begin{{column}}{{0.5\\textwidth}}
            \\centering
            \\textbf{{LD2 ({unfold_type})}}\\\\
            \\includegraphics[width=\\textwidth,height=0.75\\textheight,keepaspectratio]{{{ld2_opt}}}
        \\end{{column}}
    \\end{{columns}}
\\end{{frame}}
""")

        # --- FINAL SLIDE: DELTA CHI^2 DETERMINATION ---
        f.write(r"""
...
""")
    ...
    print(f"Successfully generated beamer source: {TEX_FILE}")
