...
def generate_latex():
    ...
    # Rule 13: LH2 Cross-sections
    for pt in pt_bins:
        ordered_files.append(f"CrossSection_LH2_pT_{pt}_Centroid_with_logo.pdf")
        ordered_files.append(f"CrossSection_LH2_pT_{pt}_GeoCenter_with_logo.pdf")

    # Rule 14: LH2 Overlay plot
    ordered_files.append("cross_section_overlay_LH2_GeoCenter.pdf") 

    # Rule 15: LD2 Cross-sections
    for pt in pt_bins:
        ordered_files.append(f"CrossSection_LD2_pT_{pt}_Centroid_with_logo.pdf")
        ordered_files.append(f"CrossSection_LD2_pT_{pt}_GeoCenter_with_logo.pdf")

    # Rule 16: LD2 Overlay plot
    ordered_files.append("cross_section_overlay_LD2_GeoCenter.pdf")
...
""" % (base_dir, title, author, inputs_and_bins_latex, event_selection_latex))
        ...
        for file in ordered_files:
            # Skip missing overlay files to prevent compilation crashes
            if "overlay" in file and not os.path.exists(os.path.join(base_dir, file)):
                continue
